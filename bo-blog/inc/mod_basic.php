    ...
    if ($categories && is_array($categories)) {
        foreach ($categories as $catearray) {
            ...
            if ($catearray['cateurl'] != '') {
                ...
                $rssurl = " <a href=\"feed.php?go=category_{$catearray['cateid']}\">
                    <img src=\"{$mbcon['images']}/rss.png\" border=\"0\" alt=\"RSS\" title=\"{$lnc[95]}\">
                </a>\n";
                $target = '';
                if ($mbcon['parentcatenum'] == '2' && $catearray['subcates'] != '') {
                    $catecount = '';
                } else {
                    $catecount = ($mbcon['showcateartnum'] == 1) ? " [" . floor($catearray['catecount']) . "]" : '';
                }
            }
...
if (in_array('statistics', $allopenmods)) {
    if (@$mbcon['stattotal'] == '1') {
        $statshow .= "{$lnc[97]} {$statistics['total']}\n<br>\n";
    }

    if (@$mbcon['stattoday'] == '1') {
        $statshow .= "{$lnc[98]} {$statistics['today']}\n<br>\n";
    }

    if (@$mbcon['statentries'] == '1') {
        $statshow .= "{$lnc[99]} {$statistics['entries']}\n<br>\n";
    }

    if (@$mbcon['statreplies'] == '1') {
        $statshow .= "<a href=\"view.php?go=comment\">{$lnc[100]} {$statistics['replies']}</a>\n<br>\n";
    }

    if (@$mbcon['stattb'] == '1') {
        $statshow .= "<a href=\"view.php?go=tb\">{$lnc[101]} {$statistics['tb']}</a>\n<br>\n";
    }

    if (@$flset['guestbook'] != 1 && $mbcon['statmessages'] == '1') {
        $statshow .= "<a href=\"guestbook.php\">{$lnc[102]} {$statistics['messages']}</a>\n<br>\n";
    }

    if (@$mbcon['statusers'] == '1') {
        $statshow .= "<a href=\"view.php?go=userlist\">{$lnc[103]} {$statistics['users']}</a>\n<br>\n";
    }

    if (@$mbcon['statonline'] == '1' && !defined('noCounter')) {
...
if (in_array('misc', $allopenmods)) {
    if ($logstat == 1) {
        $misccontent = "<a href='login.php?job=logout'>{$lnc[78]}</a>\n<br>\n";
        $misccontent .= "<a href='login.php?job=modpro'>{$lnc[90]}</a>\n<br>\n";
    } elseif ($openidloginstat == 1) {
        $misccontent = "<a href='login.php?job=logout'>{$lnc[78]}</a>\n<br>\n";
    } else {
        $misccontent = "<a href='login.php'>{$lnc[89]}</a>\n<br>\n";
        $misccontent .= "<a href='login.php?job=register'>{$lnc[79]}</a>\n<br>\n";
    }
    $misccontent .= ($permission['CP'] == 1) ? "<a href='admin.php'>{$lnc[107]}</a>\n<br>\n" : '';
    $misccontent .= ($permission['AddEntry'] == 1) ? "<a href='admin.php?act=edit'>{$lnc[108]}</a>\n<br>\n" : '';
    $misccontent .= ($permission['ApplyLink'] == 1) ? "<a href='login.php?job=applylink'>{$lnc[109]}</a>\n<br>\n" : '';
...
if (in_array('entries', $allopenmods)) {
    if (file_exists("data/cache_latest.php")) {
        include("data/cache_latest.php");
    }
    ...
    for ($i = 0; $i < count($listentriesitem); $i++) {
        ...
        $entries_list .= "  <li class='{$addintionalcssclass}'>
    <a href=\"" . getlink_entry($listentriesitems['blogid'], $listentriesitems['blogalias']) . "\" title=\"{$listentriesitems['fulltitle']}\">
      {$listentriesitems['title']}
    </a>
  </li>\n";
    }
    $entries_list .= "</ul>\n";
...
if (in_array('replies', $allopenmods)) {
    ...
    if ($listrepliesitem) {
        ...
        for ($i = 0; $i < count($listrepliesitem); $i++) {
            ...
            $replies_list .= "<li class='{$addintionalcssclass}'><a href=\"" . getlink_entry($listrepliesitems['blogid'],
                    $listrepliesitems['blogalias']) . "#blogcomment{$listrepliesitems['repid']}\" title=\"[{$listrepliesitems['replier']}] - {$listrepliesitems['title']}\">{$listrepliesitems['repcontent']}</a></li>";
        }
        $replies_list .= "</ul>";
    }
...
if ($mbcon['searchon'] == 1) {
	...
	<select name="searchmethod">
	  <option value="1">{$lnc[123]}</option>
	  <option value="2">{$lnc[124]}</option>
	  <option value="3">{$lnc[125]}</option>
	  <option value="4">{$lnc[126]}</option>
      {$addtagsearch}
    </select>
...
$blogitem['copyright'] += array(
 ...
 <span id=\"footer-security\">
   <a href=\"http://www.cnbct.org\" target=\"_blank\" title=\"Code detection by Bug.Center.Team\">
