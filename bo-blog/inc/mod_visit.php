...
if (@$flset['guestbook'] == 1 && ($job == 'addmessage' || $job == 'editmessage')) {
    getHttp404($lnc[313]);
} //2011/2/20 Forgot to close this entry
...
if ($job == 'addreply' || $job == 'addmessage' || $job == 'editreply' || $job == 'editmessage') {
    ...
    if ($job == 'addreply' || $job == 'addmessage') {
        if ($stat_html == 1 && $permission['Html'] == 1) {
            $html = 1;
        } else {
            $html = 0;
        }
        if ($stat_ubb == 1 && $permission['Ubb'] == 1) {
            $ubb = 1;
        } else {
            $ubb = 0;
        }
        if ($stat_emot == 1 && $permission['Emot'] == 1) {
            $emot = 1;
        } else {
            $emot = 0;
        }
        ...
        if (preg_search($v_replier, $forbidden['banword']) || preg_search($v_replier, $forbidden['keep'])) {
            $cancel = $lnc[158];
        }
        if (strlen($v_replier) > 45) {
            $cancel = $lnc[213];
        }
    ...
    if ($lengthContent > $permission['MaxPostLength']) {
        $cancel = $lnc[214];
    }
    ...
    if ($mbcon['censorall'] == '1') {
        ...
        if (substr_count($v_content, '[url') >= $mbcon['susurlnum']) {
            $suspectspam = 1;
        }
        if (substr_count($v_content, 'www.') >= $mbcon['susurlnum']) {
            $suspectspam = 1;
        }
        //If the post contains spam common words, it may be a spam
        if (preg_search($v_content, $forbidden['suspect'])) {
            $suspectspam = 1;
        }
        //If the post is too short, it may be a spam
        if (strlen($v_content) <= $mbcon['susminchar']) {
            $suspectspam = 1;
        }
    } else {
        $suspectspam = 0;
    ...
    if ($ajax != 'on') {
        ...
        if ($job == 'addreply' || $job == 'editreply') {
            $output_single = $m_b->single_reply($eachreply, floor($onetimecounter));
        } else {
            $output_single = $m_b->single_message($eachreply, floor($onetimecounter));
        }
...
<br>\n";
        ...
        if (!writetofile("{$db_tmpdir}/{$sid}.php", $pinchall)) {
            catcherror($lnc[226]);
        }
...
if ($job == 'viewresult') {
    acceptrequest('sid');
    $sid = basename($sid);
    if (!file_exists("{$db_tmpdir}/{$sid}.php")) {
        catcherror($lnc[226]);
    }
    ...
    if ($searchmethod == 3) {
        ...
        for ($i = 0; $i < count($records); $i++) {
            ...
            <a href=\"" . getlink_entry($records[$i]['blogid'], $records[$i]['blogalias']) . "\">
              {$records[$i]['title']}
            </a>
            <br>
            <strong>{$lnc[76]}</strong>" . $records[$i]['repcontent'];
        }
    ...
    if ($searchmethod == 5) {
        ...
        for ($i = 0; $i < count($alltags['tagname']); $i++) {
            ...
            $tag_show[] = "<a href=\"{$urlref}\" title=\"{$lnc[188]}{$alltags['tagcounter'][$i]}\" rel=\"tag\">
                {$alltags['tagname'][$i]}
            </a>";
        }
