...
switch ($job) {
    case 'default':
        ...
        if (@$flset['modeselectable'] != 1) {
            $pagebaritem = " [ {$lnc[181]} 
                <a href=\"" . sprintf($urlpattern, 1) . "\" title=\"{$lnc[182]}\">{$lnc[183]}</a> 
                | <a href=\"" . sprintf($urlpattern, 2) . "\" title=\"{$lnc[184]}\">{$lnc[185]}</a> ]";
        }
    ...
    case 'category':
        ...
        if (is_numeric($itemid)) {
            $itemid = floor($itemid);
        } elseif (isset($categorynames[$itemid])) {
            $itemid = floor($categorynames[$itemid]);
        } else {
            getHttp404($lnc[186]);
        }
