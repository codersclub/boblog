...
class template
{
    function __construct()
    {
        ...
        if (empty($elements)) {
            global $lnc, $tptvalue;
            if (!defined('elementfile') || !file_exists(elementfile)) {
                define('elementfile', 'template/default/elements.php');
            }
            if (!file_exists(elementfile)) {
                die ("Cannot find template. You may need to reinstall the program.");
            }
            include_once(elementfile);
            if (!$template['moreimages']) {
                $template['moreimages'] = "images";
            }
            if (!$template['sysver']) {
                $template['sysver'] = '5.1';
            } else {
            ...
            if (is_file("inc/tpltune/{$template['sysver']}.php")) {
                include_once("inc/tpltune/{$template['sysver']}.php");
            }
    ...
    function set($elementname, $array, $inherit = 0)
    {
        ...
        if ($a != 0) {
            ...
            eval($phpcode);
            $content[$elementname] = preg_replace("/<!-- php --><!--(.+?)--><!-- \/php -->/is", '',
                $content[$elementname]);
        }
    ...
    function publish_all($screen = false)
    { //Only works while inherit=1
        global $content;
        if (is_array($content)) {
    ...
    function showtips($title, $tips, $links = '', $enableautojump = false)
    {
        ...
        $previouspage = (empty($_SERVER['HTTP_REFERER'])) ? "javascript: history.back(1);" : $_SERVER['HTTP_REFERER'];
        $csslocation = '';
        $admin_plus = '';
        $autojump = '';
...
class getblogs extends boblog
{
    ...
    function single_record($partialquery, $partialquery2 = '')
    {
        ...
        if ($partialquery2) {
            ...
            $result[0]['previoustitle'] = ($mbcon['linklength'] == 0) ? @$previousresult['title'] : mb_substr(@$previousresult['title'],
                0, $mbcon['linklength']);
            $result[0]['previousid'] = @$previousresult['blogid'];
            $result[0]['nexttitle'] = ($mbcon['linklength'] == 0) ? @$nextresult['title'] : mb_substr(@$nextresult['title'],
                0, $mbcon['linklength']);
    ...
    function single_reply($eachreply, $i = 0)
    {
        ...
        $oddorcouplecss = ($i % 2 == 0) ? 'couple' : 'odd';
        if ($eachreply['reproperty'] == 0 || $eachreply['reproperty'] == 1) {                                                                                                                                                                                                      // A Normal Reply
            ...
            $addadminreply = $deladminreply = $editadminreply = $delreply = $blockreply = $adminrepliershow = $adminreplycontent = $adminreplybody = $replieremail = $replierhomepage = '';
            ...
            if ($permission['SeeIP'] == 1) {
                $replierip = "<a href=\"{$mbcon['ipsearch']}{$eachreply['repip']}\" target=\"_blank\" title=\"IP: {$eachreply['repip']}\"><img src=\"{$mbcon['images']}/ip.gif\" border=\"0\" alt=\"IP\"></a>\n";
            }
            if ($eachreply['repemail'] and @$permission['SeeEMAIL'] == 1) {
                $replieremail = "<a href=\"mailto:{$eachreply['repemail']}\" title=\"{$lnc[18]}\"><img src=\"{$mbcon['images']}/email.gif\" border=\"0\" alt=\"Email\"></a>\n";
            }
            if ($eachreply['repurl']) {
                $replierhomepage = "<a href=\"{$eachreply['repurl']}\" target=\"_blank\" title=\"{$lnc[19]}\"><img src=\"{$mbcon['images']}/homepage.gif\" border=\"0\" alt=\"Homepage\"></a>\n";
            }
            ...
            if ($eachreply['reproperty'] == 1 && $permission['SeeHiddenReply'] != 1) { //No permission to view
                if ($eachreply['reppsw'] != '') { //You can provide a Password to view the rep/mes
                    ...
                    $replycontent .= "<form action=\"javascript: getprotectedreply({$eachreply['repid']}, 'reply', {$i});\" method=\"post\">
<input type=\"password\" name=\"reppsw\" id=\"reppsw{$eachreply['repid']}\" maxlength='12' size='8' class='text'> 
<input type=\"submit\" value=\"{$lnc[25]}\" class='button'>
                ...
                $replycontent = $this->getcontent($eachreply['repcontent'], $eachreply['html'], $eachreply['ubb'],
                    $eachreply['emot']);
            }
            if ($userdetail['ip'] == $eachreply['repip'] && ($nowtime['timestamp'] - $eachreply['reptime'] < $mbcon['editcomment']) && $eachreply['reppsw'] == '' && empty($eachreply['adminreptime'])) {            //Allow edit
                ...
                $expirereplytime = zhgmdate("{$mbcon['timeformat']} H:i",
                    ($eachreply['reptime'] + 3600 * $config['timezone'] + $mbcon['editcomment']));
                $replycontent .= "<br>
<div class=\"commentbox-label\">
  {$lnc[300]} {$expirereplytime} {$lnc[301]}
  [<a href=\"javascript: showhidediv('editcomment{$eachreply['repid']}');\">{$lnc[302]}</a>]
</div>\n";
                $replycontent .= "<div id=\"editcomment{$eachreply['repid']}\" style='display: none;'>
<form action=\"javascript: ajax_editcomment({$eachreply['repid']}, 'reply', {$i});\" method=\"post\" id=\"formeditcomment{$eachreply['repid']}\">
  <textarea cols='55' rows='4' id=\"editcomcontent{$eachreply['repid']}\" name='v_contentc'>{$rawreplycontent}</textarea>
  <br>
  <input type='button' value='{$lnc[25]}' onclick=\"ajax_editcomment({$eachreply['repid']}, 'reply', {$i});\" class='button'> 
  <input type='reset' value='{$lnc[26]}' class='button'>
            ...
            if ($eachreply['adminreptime'] && ($eachreply['reproperty'] != 1 || $permission['SeeHiddenReply'] == 1 || $eachreply['reppsw'] == '')) {
                ...
                $adminreptime = zhgmdate("{$mbcon['timeformat']} H:i",
                    ($eachreply['adminreptime'] + 3600 * $config['timezone']));
  ...
  {$lnc[24]}
  <br>
  <textarea cols='66' rows='3' name='adminreplycontent' id='adminreplycontent{$eachreply['repid']}'>" . safe_invert($eachreply['adminrepcontent']) . "</textarea>
  <br>
  <input type='button' value='{$lnc[25]}' onclick=\"ajax_adminreply_edit('{$eachreply['repid']}', 'reply'); return false;\" class='button'>
  <input type='reset' value='{$lnc[26]}'  class='button'> 
  <input type='button' value='{$lnc[27]}' onclick=\"showhidediv('com_{$eachreply['repid']}');\" class='button'>
</form>\n";
                if ($permission['ReplyReply'] == 1) {
                    $addadminreply = "<a href=\"javascript: showhidediv('com_{$eachreply['repid']}');\">[{$lnc[20]}]</a>\n";
                }
                ...
                if ($eachreply['adminrepedittime']) {
                    $adminrepedittime = zhgmdate("{$mbcon['timeformat']} H:i",
                        ($eachreply['adminrepedittime'] + 3600 * $config['timezone']));
                    $adminreplycontent .= "<br>\n<span class='lastmodified'>{$lnc[29]} {$eachreply['adminrepeditor']} {$lnc[30]} {$adminrepedittime}</span>\n";
                }
                if ($permission['ReplyReply'] != 1) {
                    $adminreplybody = '';
                }
...
//vot			$oddorcouplecss=($i%2==0) ? 'couple' : 'odd'; //added on 2006-11-14
            ...
            $output_single = $t->set('comment', array(
                ...
                'replierip'         => $replierip,
                'replytime'         => $replytime,
                'addadminreply'     => $addadminreply,
                'deladminreply'     => $deladminreply,
                'editadminreply'    => $editadminreply,
                'delreply'          => $delreply,
                'blockreply'        => $blockreply,
                'replycontent'      => $replycontent,
                'ifadminreplied'    => $ifadminreplied,
                'adminrepliershow'  => $adminrepliershow,
                'adminreplycontent' => $adminreplycontent,
                'commentid'         => "com_{$eachreply['repid']}",
    ...
    function single_message($eachreply, $i = 0)
    {
        ...
        $replier = $replierip = $replytime = $addadminreply = $deladminreply = $editadminreply = $replycontent = $adminreplycontent = $ifadminreplied = $adminrepliershow = $adminreplycontent = $adminreplybody = $avataraddress = $avatardetail = ''; //UNSET
        $replieremail = $replierhomepage = '';
        if ($eachreply['replierid'] == -1) {
            $replier = $eachreply['replier'];
            if (@$flset['avatar'] != 1 && $mbcon['visitorgravatar'] == '1' && !empty($eachreply['repemail'])) { //Avatars for nonusers
                $avataraddress = get_gravatar($eachreply['repemail']);
        ...
        if ($permission['SeeIP'] == 1) {
            $replierip = "<a href=\"{$mbcon['ipsearch']}{$eachreply['repip']}\" target=\"_blank\" title=\"IP: {$eachreply['repip']}\">
  <img src=\"{$mbcon['images']}/ip.gif\" border=\"0\" alt=\"IP\">\n</a>\n";
        }
        $replytime = zhgmdate("{$mbcon['timeformat']} H:i",
            ($eachreply['reptime'] + 3600 * $config['timezone'])); //Need Further Change
        if ($eachreply['repemail'] and @$permission['SeeEMAIL'] == 1) {
            $replieremail = "<a href=\"mailto:{$eachreply['repemail']}\" title=\"{$lnc[18]}\">
  <img src=\"{$mbcon['images']}/email.gif\" border=\"0\" alt=\"Email\">\n</a>\n";
        }
        if ($eachreply['repurl']) {
            $replierhomepage = "<a href=\"{$eachreply['repurl']}\" target=\"_blank\"><img src=\"{$mbcon['images']}/homepage.gif\" border=\"0\" alt=\"Homepage\" title=\"{$lnc[19]}\"></a>";
        }
        if ($permission['ReplyReply'] == 1) {
        ...
        if ($eachreply['reproperty'] == 1 && $permission['SeeHiddenReply'] != 1) { //No permission to view
            if ($eachreply['reppsw'] != '') { //You can provide a Password to view the rep/mes
                $replycontent = $lnc[304];
                $replycontent .= "<form action=\"javascript: getprotectedreply({$eachreply['repid']}, 'message', {$i});\" method=\"post\">
  <input type=\"password\" name=\"reppsw\" id=\"reppsw{$eachreply['repid']}\" maxlength='12' size='8' class='text'> 
  <input type=\"submit\" value=\"{$lnc[25]}\" class='button'>
</form>\n";
            } else {
...
<div class=\"commentbox-label\">
  ...
  [<a href=\"javascript: showhidediv('editcomment{$eachreply['repid']}');\">{$lnc[302]}</a>]
            ...
            $replycontent .= "<div id=\"editcomment{$eachreply['repid']}\" style='display: none;'>
<form action=\"javascript: ajax_editcomment({$eachreply['repid']}, 'message', {$i});\" method=\"post\" id=\"formeditcomment{$eachreply['repid']}\">
  <textarea cols='55' rows='4'  id=\"editcomcontent{$eachreply['repid']}\" name='v_contentc'>{$rawreplycontent}</textarea>
  <br>
  <input type='button' value='{$lnc[25]}' onclick=\"ajax_editcomment({$eachreply['repid']}, 'message', {$i});\"  class='button'> 
  <input type='reset' value='{$lnc[26]}' class='button'>
</form>
</div>\n";
        }
  ...
  <textarea cols='66' rows='3' name='adminreplycontent' id='adminreplycontent{$eachreply['repid']}'>" . safe_invert($eachreply['adminrepcontent']) . "</textarea>
  <br>
  <input type='button' value='{$lnc[25]}' onclick=\"ajax_adminreply_edit('{$eachreply['repid']}', 'message'); return false;\" class='button'> 
  <input type='reset' value='{$lnc[26]}'  class='button'> 
  <input type='button' value='{$lnc[27]}' onclick=\"showhidediv('com_{$eachreply['repid']}');\" class='button'>
</form>\n";
            if ($permission['ReplyReply'] == 1) {
    ...
    function output($entry, $way = 'excerpt', $contentonly = false)
    {
        ...
        list($entrydatey, $entrydatem, $entrydated) = explode('/',
            gmdate('Y/n/j', ($entry['pubtime'] + 3600 * $config['timezone'])));
        ...
        $entryviews = "<a href=\"" . getlink_entry($entry['blogid'],
                $entry['blogalias']) . "\">{$lnc[35]}({$entry['views']})</a>";
        if ($entry['property'] == 1) {//Locked
            $entrycomment = "{$lnc[36]}({$entry['replies']})";
        } else {
            $entrycomment = "<a href=\"" . getlink_entry($entry['blogid'],
                    $entry['blogalias']) . "#reply\" title=\"{$lnc[37]}\">{$lnc[38]}({$entry['replies']})</a>";
        }
        ...
        if (@$flset['star'] != 1) {
            ...
            $entrystar .= ($permission['AddEntry'] == 1) ? (
            ($entry['starred'] % 2 == 1)
                ? "<a href=\"javascript: dostar('{$entry['blogid']}');\" title=\"{$lnc[39]}\"><img src=\"{$template['moreimages']}/others/starred.gif\" alt=\"\" border=\"0\"></a>"
                : "<a href=\"javascript: dostar('{$entry['blogid']}');\" title=\"{$lnc[40]}\"><img src=\"{$template['moreimages']}/others/unstarred.gif\" alt=\"\" border=\"0\"></a>"
            )
                : (
                ($entry['starred'] % 2 == 1)
                    ? "<img src=\"{$template['moreimages']}/others/starred.gif\" alt=\"\" title=\"{$lnc[39]}\" border=\"0\">"
                    : "<img src=\"{$template['moreimages']}/others/unstarred.gif\" alt=\"\" title=\"{$lnc[40]}\" border=\"0\">"
                );
            $entrystar .= "</span>\n";
        }
        ...
        if ($permission['EditEntry'] != 1) {
            ...
            if ($adminlogstat == 1) {
                $adminbar = "<div id=\"admin{$entry['blogid']}\" style=\"display: none;\" class=\"textbox-adminbar\">
<br>
<strong>{$lnc[41]}</strong>
<a href=\"admin.php?go=edit_edit_{$entry['blogid']}\">{$lnc[42]}</a> 
| <a href=\"javascript: showdelblog('{$entry['blogid']}');\">{$lnc[43]}</a> 
| <a href=\"javascript: comfirmurl('admin.php?go=entry_ae_noreply&amp;tid={$entry['blogid']}');\">{$lnc[44]}</a> 
| <a href=\"javascript: comfirmurl('admin.php?go=entry_ae_notb&amp;tid={$entry['blogid']}');\">{$lnc[45]}</a>
<br>
<strong>{$lnc[46]}</strong>
<a href=\"admin.php?go=entry_ae_lock&amp;tid={$entry['blogid']}\">{$lnc[47]}</a> 
| <a href=\"admin.php?go=entry_ae_unlock&amp;tid={$entry['blogid']}\">{$lnc[48]}</a> 
| <a href=\"admin.php?go=entry_ae_sticky1&amp;tid={$entry['blogid']}\">{$lnc[49]}</a> 
| <a href=\"admin.php?go=entry_ae_sticky2&amp;tid={$entry['blogid']}\">{$lnc[50]}</a> 
| <a href=\"admin.php?go=entry_ae_sticky0&amp;tid={$entry['blogid']}\">{$lnc[51]}</a>
<br>
<strong>{$lnc[52]}</strong>
<a href=\"javascript: comfirmurl('admin.php?go=entry_ae_noread&amp;tid={$entry['blogid']}');\">{$lnc[53]}</a> 
| <a href=\"admin.php?go=entry_ae_recountrep&amp;tid={$entry['blogid']}\">{$lnc[54]}</a> 
| <a href=\"admin.php?go=entry_ae_recounttb&amp;tid={$entry['blogid']}\">{$lnc[55]}</a>
</div>\n";
            }
        ...
        if ($way == 'excerpt' || $way == 'viewentry') {
            if ($entry['blogpsw'] && $permission['SeeAllProtectedEntry'] != 1 && $userdetail['userid'] != $entry['authorid'] && !$contentonly && $_COOKIE["entrypassword{$entry['blogid']}"] != $entry['blogpsw']) { //Password protected entry
                $entry['content'] = "<div id='protectedentry{$entry['blogid']}'>
  <div class=\"quote\">
    <div class=\"quote-title\">{$lnc[294]}</div>
    <div class=\"quote-content\">
      <form action=\"javascript: getprotectedblog({$entry['blogid']}, '{$way}');\" method='post'>
        {$lnc[296]}
        <br>
        {$lnc[133]}
        <input type='password' name='entrypsw' id='entrypsw{$entry['blogid']}' class='text'>
        <input type='submit' value='{$lnc[25]}' class='button'>
      </form>
    </div>
  </div>
                ...
                if ($way == 'excerpt') {
                    ...
                    $entry['content'] = preg_replace("/\[separator\]/", "<a name=\"entrymore\"></a>", $entry['content'],
                        1);
            ...
            if ($way == 'viewentry' && $mbcon['showtoolbar'] == '1') { //Toolbar
                ...
                $toolbar = "<img src=\"{$mbcon['images']}/toolbar_fontsize.gif\" alt='' title=\"{$lnc[61]}\" border='0'> 
  <a href=\"javascript: doZoom(16);\">{$lnc[62]}</a> 
| <a href=\"javascript: doZoom(14);\">{$lnc[63]}</a> 
| <a href=\"javascript: doZoom(12);\">{$lnc[64]}</a> 
<a href=\"feed.php?go=entry_{$entry['blogid']}\" title=\"{$lnc[65]}\">
  <img src=\"{$mbcon['images']}/toolbar_rss.gif\" alt='' border='0'>
</a>
{$toolbarplus}\n";
            }
            $entrycontent = $this->getcontent($entry['content'], $entry['htmlstat'], $entry['ubbstat'],
                $entry['emotstat'], 1);
            ...
            if (@$entry['previousid'] != '') {
                ...
                $previous = "<a href=\"{$previousentryurl}\" title=\"{$lnc[69]} {$entry['previoustitle']}\">
                        <img src=\"{$mbcon['images']}/toolbar_previous.gif\" alt='' border='0'>{$entry['previoustitle']}
                    </a>\n";
            } else {
            ...
            if (@$entry['nextid'] != '') {
                ...
                $next = "<a href=\"{$nextentryurl}\" title=\"{$lnc[70]} {$entry['nexttitle']}\">
                        <img src=\"{$mbcon['images']}/toolbar_next.gif\" alt='' border='0'>{$entry['nexttitle']}
                        </a>\n";
            } else {
                $nextentryexist = 'none';
            }
        ...
        $section_bodys[] = $t->set($way, array(
                ...
                'entryrelurl'         => get_entry_url($entry['blogid'], $entry['blogalias']),
                'entryabsurl'         => "{$config['blogurl']}/" . get_entry_url($entry['blogid'], $entry['blogalias']),
                'entrydatey'          => $entrydatey,
                'entrydatem'          => $entrydatem,
                'entrydated'          => $entrydated,
                'entrycommentnum'     => $entry['replies'],
                'entrytbnum'          => $entry['tbs'],
                'entryviewsnum'       => $entry['views'],
                'entrytbnumwithlink'  => $entrytbnumwithlink,
                'entrytburl'          => $entrytburl,
                'previousentryexist'  => $previousentryexist,
                'previousentrytitle'  => $previousentrytitle,
                'previousentryurl'    => $previousentryurl,
                'nextentryexist'      => $nextentryexist,
                'nextentrytitle'      => $nextentrytitle,
                'nextentryurl'        => $nextentryurl,
                'entrydatemnamefull'  => $entrydatemnamefull,
                'entrydatemnameshort' => $entrydatemnameshort,
                'entrysourcewithlink' => $entrysourcewithlink,
    ...
    function make_visit_form($formtitle, $id, $actionurl)
    {
        ...
        $hidden_areas = $if_securitycode_begin = $if_securitycode_end = '';
        $checked_rememberme = '';
        ...
        if ($logstat == 1) {
            ...
            $if_neednopsw_begin = '<!--';
            $if_neednopsw_end = "-->\n{$lnc[77]} <strong>{$userdetail['username']}</strong>
<a href=\"login.php?job=logout\" title=\"{$lnc[78]}\">[{$lnc[78]}]</a>\n";
            $if_neednopsw_rawend = "-->";
            ...
            $repopenurl = '';
            $hidden_areas .= "<input type=\"hidden\" name=\"v_replier\" id=\"v_replier\" value=\"{$replier}\">
<input type=\"hidden\" name=\"v_password\" id=\"v_password\" value=\"{$password}\">
<input type=\"hidden\" name=\"v_repemail\" id=\"v_repemail\" value=\"{$repemail}\">
<input type=\"hidden\" name=\"v_repurl\" id=\"v_repurl\" value=\"{$repurl}\">\n";
        } elseif ($openidloginstat == 1) {
            $repopenurl = $_COOKIE['openid_url_id'];
            $disable_replier = "disabled='disabled'";
            $if_neednopsw_begin = '<!--';
            $if_neednopsw_end = "-->\n{$lnc[77]} <strong>{$_COOKIE['openid_url_id']}</strong> 
<a href=\"login.php?job=logout\" title=\"{$lnc[78]}\">[{$lnc[78]}]</a>\n";
            $if_neednopsw_rawend = "-->";
            $disable_password = $disable_openurl = "disabled='disabled'";
            $additional = "";
            $hidden_areas .= "<input type=\"hidden\" name=\"v_replier\" id=\"v_replier\" value=\"\">
<input type=\"hidden\" name=\"v_password\" id=\"v_password\" value=\"\">
<input type=\"hidden\" name=\"v_repemail\" id=\"v_repemail\" value=\"\">
<input type=\"hidden\" name=\"v_repurl\" id=\"v_repurl\" value=\"\">
<input type=\"hidden\" name=\"openid_url\" id=\"openid_url\" value=\"{$_COOKIE['openid_url_id']}\">\n";
        } else {
        ...
        $hidden_areas .= "<input type=\"hidden\" name=\"v_id\" id=\"v_id\" value=\"{$id}\">
<input type=\"hidden\" name=\"v_reppsw\" id=\"v_reppsw\" value=\"\">\n";
        ...
        $ubbcode = "<script src=\"editor/ubb/ubbeditor_tiny.js\"></script>
<div style=\"margin: 4px 0px 4px 0px;\">
  <img src=\"editor/ubb/images/bar.gif\" alt=''> 
  &nbsp;
  <a href=\"javascript: bold();\" title=\"{$lnc[80]}\">
    <img border='0' src=\"editor/ubb/images/bold.gif\" alt=''>
  </a> 
  &nbsp;
  <a href=\"javascript: italicize();\" title=\"{$lnc[81]}\">
    <img border='0' src=\"editor/ubb/images/italic.gif\" alt=''>
  </a> 
  &nbsp;
  <a href=\"javascript: underline();\" title=\"{$lnc[82]}\">
    <img border='0' src=\"editor/ubb/images/underline.gif\" alt=''>
  </a> 
  &nbsp;
  <img src=\"editor/ubb/images/bar.gif\" alt=''> 
  &nbsp;
  <a href=\"javascript: image();\" title=\"{$lnc[83]}\">
    <img border='0' src=\"editor/ubb/images/insertimage.gif\" alt=''>
  </a> 
  &nbsp;
  <a href=\"javascript: hyperlink();\" title=\"{$lnc[84]}\">
    <img border='0' src=\"editor/ubb/images/url.gif\" alt=''>
  </a> 
  &nbsp;
  <a href=\"javascript: email();\" title=\"{$lnc[85]}\">
    <img border='0' src=\"editor/ubb/images/email.gif\" alt=''>
  </a> 
  &nbsp;
  <a href=\"javascript: quoteme();\" title=\"{$lnc[86]}\">
    <img border='0' src=\"editor/ubb/images/quote.gif\" alt=''>
  </a>
</div>\n";
        ...
        $out = $t->set("form_reply", array(
            ...
            'disable_ubb'           => $disable_ubb,
            'disable_emot'          => $disable_emot,
            'replier'               => $replier,
            'disable_replier'       => $disable_replier,
            'password'              => $password,
            ...
            'if_securitycode_begin' => $if_securitycode_begin,
            'if_securitycode_end'   => $if_securitycode_end,
            'jobnow'                => $jobnow,
            'if_neednopsw_begin'    => $if_neednopsw_begin,
            'if_neednopsw_end'      => $if_neednopsw_end,
            'if_neednopsw_rawend'   => $if_neednopsw_rawend,
            'checked_rememberme'    => $checked_rememberme,
            'ubbcode'               => $ubbcode,
            'repopenurl'            => $repopenurl,
    ...
    function make_pagebar($page, $numperline, $returnurl, $totalvolume = 'auto', $perpagevolume = 20, $pageway = 0)
    {
        ...
        $pagebar .= " <a href=\"" . sprintf($urlpattern, 1) . "\" title=\"{$lnc[9]}\">
  <img src=\"{$template['moreimages']}/arrows/doubleleft.gif\" alt=\"{$lnc[9]}\" border=\"0\">
</a>\n";
        if ($page != 1) {
            $previouspageurl = sprintf($urlpattern, ($page - 1));
            $pagebar .= "<a href=\"{$previouspageurl}\" title=\"{$lnc[10]}\">
  <img src=\"{$template['moreimages']}/arrows/singleleft.gif\" alt=\"{$lnc[10]}\" border=\"0\">
</a>\n";
        ...
        if ($page != $this->total_pages) {
            ...
            $pagebar .= " <a href=\"{$nextpageurl}\" title=\"{$lnc[11]}\">
   <img src=\"{$template['moreimages']}/arrows/singleright.gif\" alt=\"{$lnc[11]}\" border=\"0\">
 </a>\n";
   ...
   <img src=\"{$template['moreimages']}/arrows/doubleright.gif\" alt=\"{$lnc[12]}\" border=\"0\"></a>\n";
    ...
    function make_innerpagebar($page, $returnurl, $totalvolume, $pageway = 0)
    {
        ...
        for ($i = 1; $i <= $totalvolume; $i++) {
            if ($i != $page) {
                $pagebar .= ($pageway == 0) ? " <a href=\"{$returnurl}{$conxer}part={$i}\">[{$i}]</a> " : " <a href=\"" . str_replace('%s',
                        $i, $returnurl) . "\">[{$i}]</a> ";
            } else {
                $pagebar .= " [{$i}] ";
            }
    ...
    function output_page($entry)
    {
        ...
        list($entrydatey, $entrydatem, $entrydated) = explode('/',
            gmdate('Y/n/j', ($entry['pagetime'] + 3600 * $config['timezone'])));
        ...
        $entrycontent = $this->getcontent($entry['pagecontent'], $entry['htmlstat'], $entry['ubbstat'],
            $entry['emotstat'], 1);
        $entrycontent = $this->keep_htmlcode_matches($entrycontent);
        ...
        $section_bodys[] = $t->set('viewpage', array(
            ...
            'entryauthor'    => $entryauthor,
            'entrycontent'   => $entrycontent,
            'entrytitletext' => $entry['pagetitle'],
            'entrydatey'     => $entrydatey,
            'entrydatem'     => $entrydatem,
            'entrydated'     => $entrydated,
        ));
