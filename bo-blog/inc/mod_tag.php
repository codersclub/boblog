...
/*vot*/ if (@$flset['tags']==1) getHttp404($lnc[313]);
...
/*vot*/ $alltagcounter=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}tags`");
/*vot*/	for ($i=0; $i<@count(@$alltags['tagid']); $i++) {
...
/*vot*/		$counter_now=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}blogs` WHERE `blogid` IN ({$entries_query}) AND `property`<'2'");
...
/*vot*/	if ($mbcon['tag_list']==1) $mainbody=$t->set('contentpage', array('title'=>"Tags: {$allentries[0]['tagname']}",  'contentbody'=>@implode('', $section_body_main)));
	else $mainbody=@implode('', $section_body_main);
/*vot*/ 	$pagetitle="Tags: {$allentries[0]['tagname']} - ";
	announcebar();
