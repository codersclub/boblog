...
if ($job == 'default') {
    ...
    for ($i = 0; $i < @count(@$alltags['tagid']); $i++) {
        ...
        if ($mbcon['tagunderlinetospace'] == 1) {
            $eachtag = str_replace('_', ' ', $alltags['tagname'][$i]);
        } else {
            $eachtag = $alltags['tagname'][$i];
        }
        $urlref = getlink_tags(urlencode(urlencode($alltags['tagname'][$i])));
        if ($alltags['tagcounter'][$i] < 0) {
            $alltags['tagcounter'][$i] = '?';
        }
        $tag_show[] = "<a href=\"{$urlref}\" title=\"{$lnc[188]}{$alltags['tagcounter'][$i]}\">
  <span style=\"font-size: {$bit_tag_size}px;\">{$eachtag}</span>
</a>\n";
    }
...
if ($job == 'show') {
    ...
    if (!is_array($allentries[0]) || $allentries[0]['tagentry'] == '<end>' || $allentries[0]['tagcounter'] == 0) {
        ...
        if (@$flset['modeselectable'] != 1) {
            $pagebar .= " [ 
                {$lnc[181]} 
                <a href=\"" . getlink_tags(urlencode(urlencode($tag)), '1') . "\" title=\"{$lnc[182]}\">{$lnc[183]}</a> 
              | <a href=\"" . getlink_tags(urlencode(urlencode($tag)), '2') . "\" title=\"{$lnc[184]}\">{$lnc[185]}</a> 
              ]";
        }
    }
    $iftoppage = ($mbcon['pagebarposition'] == 'down') ? 'none' : 'block';
