...
if ($langback == 'ru') {
    include_once("plugin/viewstat/lang_ru.php");
} else {
    if ($langback == 'en') {
        include_once("plugin/viewstat/lang_en.php");
    } else {
        if ($langback == 'zh-tw') {
...
for ($i = 0; $i < $thesedays; $i++) {
    $day_his[$i] = substr($stathistory_all[$i]['hisday'], 0, 4) . '-' . substr($stathistory_all[$i]['hisday'], 4,
            2) . '-' . substr($stathistory_all[$i]['hisday'], 6, 2);
    $vis_his[$i] = $stathistory_all[$i]['SUM(visit)'];
    $ruler = max($vis_his[$i], $ruler);
}

for ($i = 0; $i < $thesedays; $i++) {
    $putgraph .= "<tr><td width=\"120\">" . $day_his[$i] . "</td><td>" . statgraphic($vis_his[$i],
            $ruler) . "</td></tr>";
}
...
function statgraphic($visitnum, $ruler)
{
    ...
    $out = "<table width=\"90%\" cellpadding=\"0\" cellspacing=\"0\" height=\"10\">
  <tr>
    <td width=\"$width\" background=\"plugin/viewstat/bar.gif\" style=\"background-repeat: repeat-x;\">&nbsp;</td>
    <td align=\"right\">$visitnum</td>
  </tr>
</table>";
    return $out;
}
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  <tr>
    <td align="center" class="sect">
      <table width=90% align=center>
        <tr>
          <td width="120">$langstat[1]</td>
          <td colspan="3">{$config['blogname']}</td>
        </tr>
        <tr>
          <td width="120">$langstat[3]</td>
          <td>$crtime</td>
          <td width="120">$langstat[4]</td>
          <td>$days $langstat[5]</td>
        </tr>
        <tr>
          <td width="120">$langstat[6]</td>
          <td>{$statistics['entries']}</td>
          <td width="120">$langstat[7]</td>
          <td>$av_art</td>
        </tr>
        <tr>
          <td width="120">$langstat[8]</td>
          <td>{$statistics['replies']}</td>
          <td width="120">$langstat[7]</td>
          <td>$av_rep</td>
        </tr>
        <tr>
          <td width="120">$langstat[9]</td>
          <td>{$statistics['users']}</td>
          <td width="120">$langstat[7]</td>
          <td>$av_reg</td>
        </tr>
        <tr>
          <td width="120">$langstat[2]</td>
        ...
        <tr>
          <td>
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td width="120">$langstat[11]</td>
                <td>{$statistics['total']} ({$langstat[7]} $av_vis)</td>
              </tr>
              <tr>
                <td width="120">$langstat[12]</td>
                <td>{$statistics['today']}</td>
              </tr>
            </table>
            ...
            <div align=center>
              $langstat[18]
              <input type=text size=4 maxlength=4 value="{$nowtime['year']}" name='clearyear'>$langstat[23]
              <input type=text size=2 maxlength=2 value="{$nowtime['month']}" name='clearmonth'>$langstat[24]
              <input type=text size=2 maxlength=2 value="{$nowtime['day']}" name='clearday'>$langstat[25]
              $langstat[26]
              <input type=hidden value="1" name='cleardata'>
              <input type=submit value='$langstat[27]'>
            </form>
          </td>
        </tr>
      </table>
