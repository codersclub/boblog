...
function chkDelItem(index) {
    if (confirm(jslanfp[3])) {
        delItem(index.parentNode.parentNode.rowIndex);
    }
}

function delItem(index) {
    dyn_t.deleteRow(index);
}

function setPref(objSet, prefName, pref) {
    return objSet.setAttribute(prefName, pref);
}
...
function add_fpRow(inum, identifier, title, creator, location, info, image, album, meta) {
    ...
    myCell.setAttribute("valign", "top");
    if ((dyn_t.rows.length % 2) == 0) {
        myCell.style.background = "#C0C0C0";
    }
}
...
function moveItem(act, index) {
    ...
    if (act == "up" & index > 0 | act == "down" & index < dyn_t.rows.length - 1) {
        ...
        switch (act) {
            case "up":
                setIndex = Array(index - 1, index, index + 1);
                break;
            case "down":
                setIndex = Array(index + 2, index, index);
                break;
        }
