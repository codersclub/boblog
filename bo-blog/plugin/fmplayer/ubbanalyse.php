...
{
    $str = preg_replace_callback("/\s*\[fmp(,id=.+?)?(,url=.+?)?\](.*?)\[\/fmp\]\s*/is", "makeFMPlayer", $str);
    return $str;
}
...
function makeFMPlayer($match)
{
    ...
    if ($RunID <= 5) {
        switch ($RunID) {
            case 5:
            case 4:
                $url = str_replace(',url=', '', strtolower($url));
                break;
            case 3:
            case 2:
                $url = "plugin/fmplayer/playlist.php?id=" . str_replace(',id=', '', strtolower($tid));
                break;
            case 1:
                $url = $str;
                $str = "";
