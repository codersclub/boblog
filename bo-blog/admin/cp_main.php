...
if ($job == 'default') {
    ...
    if (@$flset['guestbook'] != 1) {
        $pending_messages = $blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}messages` WHERE `reproperty`=2 OR `reproperty`=3");
    }
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2  class="sect">
      ...
      <b>{$lna[51]}</b> {$phpver} &nbsp;&nbsp; [<a href="admin.php?go=main_phpinfo" target="_blank">{$lna[55]}</a>]
      <br>
      <b>{$lna[52]}</b> {$phpos}
      <br>
      <b>{$lna[53]}</b> {$mysqlver}
...
<!--
      <div align=center>
        <a href="admin.php?go=main_phpinfo" target="_blank">{$lna[55]}</a>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <a href="admin.php?go=main_dbstat">{$lna[56]}</a>
        </div>
-->
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2  class="sect">
      <b>Version {$blogversion}</b> ({$codeversion})
      <br>
      <br>
      <div style="height: 100px; width: 500px; overflow:auto; border: 1px solid #CCC">
        <b>Core Team</b>
        <br>
        <b>Chief Developer:</b> Bob
        <br>
        <b>Plugins:</b> Mesak Chuan
        <br>
        <b>UI Designers:</b> guihuo
        <br>
        <b>Supports:</b> Totti, Lang Man Hai Er, yeyo, Mahayu
        <br>
        <br>
        <b>Localization</b>
        <br>
        <b>English:</b> Bob
        <br>
        <b>Russian:</b> Valery Votintsev
        <br>
        <b>Traditional Chinese:</b> Mesak Chuan
        <br>
        <b>Vietnamese:</b> Meng Ling, kusanagi
        <br>
        <br>
        <b>Security</b>
        <br>
        <b>Code Detection:</b> Bug.Center.Team
        <br>
        <br>
...
if ($job == 'dbstat') {
    ...
    while (false !== ($line_t = db_fetch_array($results_t)) && strstr($line_t['Name'], $db_prefix)) {
        $table_infos .= "<tr><td>{$line_t['Name']}</td><td>{$line_t['Rows']}</td>	<td>" . round($line_t['Data_length'] / 1024,
                2) . " KB</td><td>" . round($line_t['Index_length'] / 1024, 2) . " KB</td></tr>";
        $datalengthall += $line_t['Data_length'];
        $indexlengthall += $line_t['Index_length'];
    }
    $table_infos .= "<tr class='sect'><td><b>{$lna[62]}</b></td><td>&nbsp;</td><td>" . round($datalengthall / 1024,
            2) . " KB</td><td>" . round($indexlengthall / 1024, 2) . " KB</td></tr>";
    $table_infos .= "</table>";
    $display_overall .= highlightadminitems('default', 'main');
...
<table class='tablewidth' cellpadding=4 cellspacing=1 align=center>
  ...
  <tr>
    <td class="prefright" align="center" colspan='2'>
      <table width=100%>
        {$pref_quicksel}
      </table>
    </td>
  ...
  $pref_result_show
  <tr>
    <td class="prefleft" valign="top" width="180">{$lna[527]}</td>
    <td class="prefright">
      <input type="button" value="{$lna[1083]}" onclick="window.location='admin.php?go=misc_urlrewrite';" class="formbutton">
    </td>
  </tr>
</table>

<br>
<input type='hidden' name='prefconfig[urlrewritemethod]' value="{$config['urlrewritemethod']}">
...
if ($job == 'configsave') {
    ...
    if (!writetofile("data/configbakup.php", $savetext)) {
        catcherror($lna[66] . "data/configbakup.php");
    }
...
if ($job == 'mbconsave') {
    ...
    if (!writetofile("data/mod_configbakup.php", $savetext)) {
        catcherror($lna[66] . "data/mod_configbakup.php");
    }
...
if ($job == 'module') {
    ...
    if ($section == 'header' || $section == 'sidebar' || $section == 'footer' || $section == 'prebody') {
        $formbody = "
<form action=\"admin.php?go=main_modulesave\" method=\"post\" id=\"ajaxForm1\">
<table width=95% align=right cellpadding=4 cellspacing=1>
  <tr class='admintitle'>
    <td width=30 align=center>{$lna[75]}</td>
    <td align=center>{$lna[76]}</td>
    <td align=center>{$lna[77]}</td>
    <td align=center>{$lna[78]}</td>
  </tr>\n";
        ...
        for ($i = 0; $i < count($mod_array); $i++) {
            ...
            $formbody .= "
  <tr class='$class_distinct'>
    <td width=30 align=center>
      <input type=checkbox name='selid[]' value='{$mod_array[$i]['name']}'{$chex}>
    </td>
    <td width='80%'>
      <b>{$mod_array[$i]['name']}</b>$addwords
      <br>
      {$mod_array[$i]['desc']}
    </td>
    ...
    <td align=center width=30>
      <a href='$linkdel'>
        <img src='admin/theme/{$themename}/del.gif' alt='{$lna[78]}' title='{$lna[78]}' border='0'>
      </a>
    </td>
  </tr>\n";
        }
        ...
        $formbody .= "
  <tr class='sect' align=center>
    <td colspan=4>
      <input type=hidden name=section value='$section'>
      <input type=button value=\"{$lna[82]}\" class='formbutton' onclick=\"adminSubmitAjax(1);\">
      <input type=reset value=\"{$lna[65]}\" class='formbutton'>
      <input type=button value=\"{$lna[83]}\" onclick=\"window.location='admin.php?go=main_module&section=new2&newitemposition={$section}'\" class='formbutton'>
      <input type=button value=\"{$lna[84]}\" onclick=\"window.location='admin.php?go=main_ordermodule&section={$section}'\" class='formbutton'>
    </td>
  </tr>
  <tr class='sect'>
    <td colspan=4>
    ...
    if ($section == 'new') {
        $formbody .= "
<table width=95% align=right cellpadding=4 cellspacing=1>
<form action=\"admin.php?go=main_module&section=new2\" method=\"post\" id=\"ajaxForm1\">
  <tr class='admintitle'>
    <td align='center' colspan=2>1. {$lna[89]}</td>
  </tr>\n";
        $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[90]}</td>
    <td class='hiddenitem' valign=top>
      <select name='newitemposition'>
        <option value='header' selected>{$lna[71]}</option>
        <option value='sidebar'>{$lna[72]}</option>
        <option value='footer'>{$lna[73]}</option>
        <option value='prebody'>{$lna[1055]}</option>
      </select>
    </td>
  </tr>\n";
        $formbody .= "
  ...
  <form enctype='multipart/form-data' action=\"admin.php?go=main_automod\" method=\"post\">
  <tr class='admintitle'>
    <td align='center' colspan=2>2. {$lna[91]}</td>
  </tr>\n";
        $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[92]}</td>
    <td class='hiddenitem' valign=top>
      <input type='file' name='newmodfile'><br>{$lna[93]}
    </td>
  </tr>\n";
        $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[482]}</td>
    <td class='hiddenitem' valign=top>
      <input type='password' name='modpassword'>
      <br>
      {$lna[964]}
    </td>
  </tr>\n";
  ...
  <tr class='admintitle'>
    ...
    if ($section == 'new2' || $section == 'edit') {
        ...
        if ($detail['func'] == 'system') {
            $ddshow = $lna[81];
        } else {
            switch ($newitemposition) {
                case 'header':
                    if ($func_target) {
                        $selected["x{$func_target}"] = 'selected';
                    } else {
...
<table width=95% class='sect'>
  <tr>
    <td width=25%>{$lna[95]}</td>
    <td><input type='text' name='func_text' size='30' value='{$func_text}'></td>
  </tr>
  <tr>
    <td width=25%>{$lna[96]}</td>
    <td>
      <input type='text' name='func_url' size='30' value='{$func_url}'>
    </td>
  </tr>
  <tr>
    <td width=25%>{$lna[97]}</td>
    <td>
      <select name='func_target'>
        <option value='_self' {$selected['x_self']}>{$lna[98]}</option>
        <option value='_blank' {$selected['x_blank']}>{$lna[99]}</option>
        <option value='_parent' {$selected['x_parent']}>{$lna[100]}</option>
      </select>
    </td>
  </tr>
  <tr>
    <td width=25%>{$lna[101]}</td>
    <td>
      <input type='text' name='func_title' size='30' value='{$func_title}'> {$lna[102]}
    </td>
  </tr>
</table>\n";
                    break;
...
<table width=95% class='sect'>
  <tr>
    <td width=25%>{$lna[104]}</td>
    <td>
      <input type='text' name='func_title' size='30' value='{$func_title}'>
      <br>
      {$lna[105]}
    </td>
  </tr>
  <tr>
    <td width=25%>{$lna[106]}</td>
    <td>
      <input type='radio' name='func_extend' value='1' {$box_1}>{$lna[107]}
      <input type='radio' name='func_extend' value='0' {$box_0}>{$lna[108]}
    </td>
  </tr>
  <tr>
    <td width=25%>{$lna[109]}</td>
    <td>
      <textarea name='func_content' cols=40 rows=8>{$func_content}</textarea>
      <br>
      {$lna[110]}
    </td>
  </tr>
...
<table width=95% class='sect'>
  <tr>
    <td width=25%>{$lna[109]}</td>
    <td>
      <textarea name='func_code' cols=40 rows=8>{$func_code}</textarea>
      <br>
      {$lna[110]}
    </td>
  </tr>
  ...
  <tr>
    ...
    <td>
      ...
      {$lna[110]}
        ...
        $formbody .= "
<form action=\"admin.php?go=main_{$actionform}\" method=\"post\" id=\"ajaxForm1\">
<table width=95% align=right cellpadding=4 cellspacing=1>
  <tr class='admintitle'>
    <td align='center' colspan=2>{$lna[113]}</td>
  </tr>\n";
        /*
        $formbody.="
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[482]}</td>
    <td class='hiddenitem' valign=top>
      <input name='modpassword' size=20 type='password'>
      <br>
      {$lna[964]}
    </td>
  </tr>\n";
  ...
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[90]}</td>
    <td class='hiddenitem' valign=top>
      {$realname[$newitemposition]}
      <input type=hidden name=newitemposition value={$newitemposition}>
    </td>
  </tr>\n";
        if ($section == 'new2') {
            $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[114]}</td>
    <td class='hiddenitem' valign=top>
      <input name='newitemname' size=20 type='text' value='{$newitemname}'>
      <br>
      {$lna[115]}
    </td>
  </tr>\n";
  ...
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[114]}</td>
    <td class='hiddenitem' valign=top>
      <input name='newitemname' type='hidden' value='{$newitemname}'>{$newitemname}
    </td>
  </tr>\n";
        }
        $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[116]}</td>
      <td class='hiddenitem' valign=top>
        <input name='newitemdesc' size=40 type='text' value='{$newitemdesc}'>
        <br>
        {$lna[117]}
      </td>
    </tr>\n";
        ...
        $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[118]}</td>
    <td class='hiddenitem' valign=top>
      <input name='newitemactive' type='checkbox' value='1' {$box_3}>{$lna[119]}
    </td>
  </tr>\n";
        $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[75]}</td>
    <td class='hiddenitem' valign=top>
      <select name='indexonly'>
        <option value=''>{$lna[1182]}</option>
        <option value='1' {$box_5}>{$lna[1152]}</option>
        <option value='2' {$box_6}>{$lna[1181]}</option>
      </td>
    </tr>\n";
        $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[186]}</td>
    <td class='hiddenitem' valign=top>
      {$lna[930]}
      <br>
      {$permitgpshow}
        ...
        $formbody .= "
  <tr>
    <td align='center' class='prefleft' width=20% valign=top>{$lna[120]}</td>
    <td class='hiddenitem' valign=top>{$ddshow}</td>
  </tr>\n";
        $formbody .= "
  <tr class='admintitle'>
    <td align='center' colspan=2>
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  <tr>
    <td width=150 class="sect" valign=top>

      <table width=100% cellpadding=4 cellspacing=1>
        <tr>
          <td class="{$class_head}" align=center height=40><a href="admin.php?go=main_module&section=header">{$lna[71]}</a></td>
        </tr>
        <tr>
          <td class="{$class_side}" align=center height=40><a href="admin.php?go=main_module&section=sidebar">{$lna[72]}</a></td>
        </tr>
        <tr>
          <td class="{$class_footer}" align=center height=40><a href="admin.php?go=main_module&section=footer">{$lna[73]}</a></td>
        </tr>
        <tr>
          <td class="{$class_prebody}" align=center height=40><a href="admin.php?go=main_module&section=prebody">{$lna[1055]}</a></td>
        </tr>
        <tr>
          <td class="{$class_new}" align=center height=40><a href="admin.php?go=main_module&section=new">{$lna[113]}</a></td>
        </tr>
      </table>
...
if ($job == 'modulesave') {
    ...
    if (!$section) {
        @header("Location: admin.php?go=main_module");
    }
    ...
    if ($count_selid == 0) {
        $blog->query("UPDATE `{$db_prefix}mods` SET `active`=0 WHERE `position`='{$section}'");
    } else {
...
if ($job == 'modulenew' || $job == 'moduledoedit') {
    ...
    if (!$newitemname || !$newitemposition || !$newitemdesc) {
        catcherror($lna[124]);
    }
    ...
    if ($indexonly) {
        $value .= "'indexonly'=>{$indexonly}, ";
    }

    if ($try['func'] != 'system') {
        switch ($newitemposition) {
            case 'header':
                if (!$func_text || !$func_url || !$func_target) {
                    catcherror($lna[124]);
                }
                $value .= "'type'=>'link', 'url'=>'" . safe_convert($func_url) . "', 'text'=>'" . safe_convert($func_text) . "'";
                if ($func_target != '_self') {
                    $value .= ", 'target'=>'{$func_target}'";
                }
                if ($func_title) {
                    $value .= ", 'title'=>'" . safe_convert($func_title) . "'";
                }
                break;
            case 'sidebar':
                if (!$func_title || !$func_content) {
                    catcherror($lna[124]);
                }
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr class='admintitle'>
    <td align='center'>
      <input type=submit value='{$lna[64]}' class='formbutton'>
      <input type=button onclick='window.location="admin.php";' value='{$lna[138]}' class='formbutton'>
    </td>
  </tr>
</table>
</form>
...
<table width=95% align=center>
  <tr>
    <td width=20%>{$lna[131]}</td>
    <td>{$info['name']}</td>
  </tr>
  <tr>
    <td width=20%>{$lna[132]}</td>
    <td>{$info['author']}</td>
  </tr>
  <tr>
    <td width=20%>{$lna[133]}</td>
    <td>{$info['time']}</td>
  </tr>
  <tr>
    <td width=20%>{$lna[134]}</td>
    <td>{$info['intro']}</td>
  </tr>
</table>
<br>
...
if ($job == 'langset') {
    ...
    while (false !== ($file = readdir($handle))) {
        if (is_dir("lang/{$file}")) {
            if ($file == $langfront) {
                $alllanglist[] = array('ldir' => $file, 'lname' => $langname_tmp['front']);
...
if ($job == 'dolangset') {
    ...
    if (!file_exists("lang/{$newlangf}/common.php")) {
        catcherror($lna['front_lang_not_found']);
    }
...
if ($job == 'funclock') {
    ...
    $uidesc = array(
        ...
        'star'           => $lnc[93],
        'guestbook'      => $lnc[91],
        'modeselectable' => "{$lnc[183]}/{$lnc[185]}",
    );
...
<table class='tablewidth' cellpadding=4 cellspacing=1 align=center>
  $pref_result_show
  <tr>
    <td colspan=2 class="sect">{$lna[1196]}</td>
  </tr>
</table>
