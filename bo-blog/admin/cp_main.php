...
if ($job=='default') {
	...
	if (file_exists("lang/{$langback}/licence")) $copytxt=readfromfile("lang/{$langback}/licence");
	else $copytxt=readfromfile("admin/licence");
	$display_overall.=highlightadminitems('default', 'main');
$display_overall.= <<<eot
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
<form action='' method='post' id='cpbox'>
<tr>
...
<textarea style="width: 80%; height: 90px;" onblur='document.getElementById("cpbox").reset();'>
{$copytxt}
</textarea>
...
<ul>
	...
	<li>{$lna[874]} <select name=targetskin>{$admskinsel}</select> <input type='submit' value='$lna[64]'> <input type='button' value='{$lna[875]}' onclick='window.location="admin.php?go=main_refreshadminskinlist";' class='button'></li>
</td>
</tr>
</form></table>
<br><br>
...
<script type="text/javascript" src="{$config['updatesrc']}/updatejs.php?v={$codeversion}&lang={$langback}"></script>
eot;
}
...
$pref_result_show
<tr><td class="prefleft" valign="top" width="180">{$lna[527]}</td><td class="prefright"><input type="button" value="{$lna[1083]}" onclick="window.location='admin.php?go=misc_urlrewrite';"></td></tr>
</table>
<br><input type='hidden' name='prefconfig[smarturl]' value="{$config['smarturl']}"><input type='hidden' name='prefconfig[urlrewrite]' value="{$config['urlrewrite']}">
...
if ($job=='module') {
	...
	if ($section=='header' || $section=='sidebar' || $section=='footer' || $section=='prebody') {
		...
		$mod_array=$blog->getgroupbyquery("SELECT * FROM `{$db_prefix}mods` WHERE `position`='{$section}' ORDER BY `order`");
	...
	if ($section=='new') {
		...
		$formbody.="<tr class='admintitle'><td align='center' colspan=2><input type=submit value='{$lna[64]}'> <input type=reset value='{$lna[65]}'></td></tr></form>\n";
	...
	if ($section=='new2' || $section=='edit') {
		...
		$formbody.="<tr class='admintitle'><td align='center' colspan=2><input type=submit value='{$lna[64]}'> <input type=reset value='{$lna[65]}'></td></tr>\n";
...
if ($job=='modulenew' || $job=='moduledoedit') {
	acceptrequest('newitemname,newitemposition,newitemdesc,newitemactive,modpassword,permitgp');
	...
	if ($job=='modulenew') {
		...
		mod_replace ($newitemname ,$workout);
		recache_mods ();
	}
	catchsuccess ($finishok2, $backtomodule);
}
...
if ($job=='automod') {
	...
	if ($ext!='.blog') catcherror($lna[129]);
...
<tr class='admintitle'><td align='center'><input type=submit value='{$lna[64]}'> <input type=button onclick='window.location="admin.php";' value='{$lna[138]}'></td></tr>
</form>
</td></tr></table>
eot;
}
...
if ($job=='ordermodule') {
	...
	$mod_array=$blog->getgroupbyquery("SELECT * FROM `{$db_prefix}mods` WHERE `position`='{$section}' AND `active`=1 ORDER BY `order`");
	for ($i=0; $i<count($mod_array); $i++) {
...
if ($job=='modorder') {
	...
	for ($i=0; $i<($lastcount-1); $i++) {
		$blog->query("UPDATE `{$db_prefix}mods` SET `order`='{$i}' WHERE `name`='{$array_cates[$i]}' AND `position`='{$section}'");
	}
	recache_mods();
	catchsuccess ($finishok2, $backtomodule);
}
...
<b>Front-End/前台/前臺</b> lang/<input style="text-align:center;" type=text name=newlangf value='{$langfront}'>/common.php <br>
<b>Back-End/后台/後臺</b> lang/<input style="text-align:center;" type=text name=newlangb value='{$langback}'>/backend.php <br>
<br>
<input type='submit' value='{$lna[64]}'> <input type='reset' value='{$lna[65]}'><br>
</td></tr>
</table>
</td></tr>
</form>
