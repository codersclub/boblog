...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  <tr align=center class="admintitle">
    <td width=35 align=center>{$lna[439]}</td>
    <td align=center>{$lna[440]}</td>
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr align=center class="sect">
    <td align=center colspan=2>
      {$lna[443]} <input type=text name='gpname' value=''>
...
if ($job == 'delgp') {
    ...
    foreach ($usergp as $key => $value) {
        if ($key != $itemid) {
            $gplist .= "<option value='{$key}'>{$value}</option>\n";
        }
    }
    @header("Content-Type: text/html; charset=utf-8");
    $t = new template;
    $htm = "<br>
<form action='admin.php?go=user_dodelgp_{$itemid}' method='post'>
  {$lna[455]}
  <br>
  <br>
  <select name='togp'>
  ...
  <input type=button value='{$lna[138]}' onclick='window.location=\"admin.php?go=user_usergroup\"' class='formbutton'>
  <input type=submit value='{$lna[64]}' class='formbutton'>
</form>";
    $t->showtips($lna[454], $htm);
    exit();
}
...
if ($job == 'users') {
  ...
  <tr class='$addclass'>
    <td align='center'>
      <input type='checkbox' name='selid[]' id='selid[]' value='{$detail_array[$i]['userid']}'>
    </td>
    <td>{$tmp_sgp}</td>
    <td align='center'>{$detail_array[$i]['userid']}</td>
    <td width=50%>{$detail_array[$i]['username']}</td>
    <td align='center'>{$tmp_tm}</td>
    <td align='center'>
      <a href='javascript: redirectcomfirm (\"admin.php?go=user_deluser_{$detail_array[$i]['userid']}\");'>
    ...
    $pagebar = gen_page($page, 5, "admin.php?go=user_users&usergroup={$usergroup}&ordered={$ordered}",
        $statistics['users'], $adminitemperpage);
  ...
  <tr>
    <td colspan=7>
      <select name="usergroup">
        <option value=''>{$lna[457]}</option>
        $puttingcates
      </select>
      <input type=submit value="{$lna[244]}" class='formbutton'>
      {$lna[458]}
      <a href="admin.php?go=user_users&usergroup={$usergroup}&ordered=1">{$lna[459]}</a>
      | <a href="admin.php?go=user_users&usergroup={$usergroup}&ordered=2">{$lna[460]}</a>
      | <a href="admin.php?go=user_users&usergroup={$usergroup}&ordered=3">{$lna[461]}</a>
      | <a href="admin.php?go=user_users&usergroup={$usergroup}&ordered=4">{$lna[462]}</a>
    </td>
  </tr>
</table>
</form>
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  <tr align=center class="admintitle">
    ...
    <td width=80>{$lna[463]}</td>
    <td width=40>{$lna[471]}</td>
    <td align=center>{$lna[464]}</td>
    <td width=190 align=center>{$lna[465]}</td>
    <td width=35 align=center>{$lna[78]}</td>
    <td width=35 align=center>{$lna[77]}</td>
  </tr>
  {$tablebody}
  ...
  <tr class="adminoption">
    <td colspan=8>
      ...
      <input type=radio name=opt value='newusergroup'>{$lna[466]}
      <select name="tousergroup">
        $puttingcates
      </select>
  ...
  <tr>
    <td colspan=2 class='sect'>
      ...
      <table class='tablewidth' align=center>
        <tr>
          <td width=25%>{$lna[469]}</td>
          <td>
            <input type='text' name='detailuser' size='20'>
          </td>
        </tr>
        <tr>
          <td width=25%>{$lna[470]}</td>
          <td>
            <input type='radio' checked=checked name='detailtype' value='username'>{$lna[464]}
            &nbsp;
            <input type='radio' name='detailtype' value='userid'>{$lna[471]}
          </td>
        </tr>
        <tr>
          <td width=25%>{$lna[472]}</td>
          <td>
            <input type='radio' checked=checked name='detailact' value='edituser'>{$lna[77]}
            &nbsp;
            <input type='radio' name='detailact' value='deluser'>{$lna[78]}
            [<b>{$lna[473]}</b>]
          </td>
        </tr>
        <tr>
