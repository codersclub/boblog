...
if ($job=='batch') {
	...
	if ($opt=='del' || $opt=='deldraft') {
		...
		if ($opt=='del') {
			...
			$blog->query($queryact);
/*vot*/			$countreps=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}replies` WHERE `reproperty`<>2 AND `reproperty`<>3 AND `reproperty`<>4");
	...
	if ($opt=='noreply') {
/*vot*/		$countreps=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}replies` WHERE `reproperty`<=1");
		$blog->query("UPDATE `{$db_prefix}counter` SET `replies`='{$countreps}'");
...
if ($job=='ae') {
	...
	switch ($itemid) {
		...
		case 'noread':
			...
			break;
		case 'recountrep':
/*vot*/			$countreps=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}replies` WHERE `reproperty`<=1 AND `blogid`='{$tid}'");
			$queryact="UPDATE `{$db_prefix}blogs` SET `replies`='{$countreps}' WHERE `blogid`='{$tid}'";
			break;
		case 'recounttb':
/*vot*/			$countreps=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}replies` WHERE `reproperty`=4 AND `blogid`='{$tid}'");
...
/*vot*/		$countreps=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}replies` WHERE `reproperty`<=1");
	...
	if ($itemid=='notb') {
/*vot*/		$countreps=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}replies` WHERE `reproperty`=4");
