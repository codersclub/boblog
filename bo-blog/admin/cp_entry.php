...
if ($job == '' || $job == "default") {
  ...
  <option value=365 {$timeperiodsel[365]}>{$lna[1136]}</option>
    ...
    $pagebar = gen_page($page, 5,
        "admin.php?go=entry_default&category={$category}&property={$property}&timeperiod={$timeperiod}&keyword=" . urlencode($keyword),
        $numenries, $adminitemperpage);
  ...
  <tr>
    <td colspan=7>
      <select name="category">
        <option value=''>{$lna[328]}</option>
        $puttingcates
      </select>
      $adminselection2
      $adminselection4
      $adminselection3
      <input type=submit value="{$lna[244]}" class='formbutton'>
    </td>
  </tr>
  <tr>
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  <tr align=center class="admintitle">
    <td width=35 align=center>{$lna[245]}</td>
    <td width=35>{$lna[297]}</td>
    <td align=center>{$lna[284]}</td>
    <td width=200 align=center>{$lna[288]}</td>
    <td width=80 align=center>{$lna[285]}</td>
    <td width=35 align=center>{$lna[78]}</td>
    <td width=35 align=center>{$lna[77]}</td>
  </tr>
  {$tablebody}
  <tr>
    <td colspan=3>
    ...
    <td colspan=4 align=right>$pagebar</td>
  </tr>
  <tr>
    <td colspan=7 height=20></td>
  </tr>
  <tr class="adminoption">
    <td colspan=7>
      {$lna[249]}
      <br>
      <input type=radio name=opt value='del' onclick="document.getElementById('f_s').submit();">{$lna[78]}
      <input type=radio name=opt value='noreply'onclick="document.getElementById('f_s').submit();">{$lna[329]}
      <input type=radio name=opt value='sticky'>{$lna[330]}
      <input type=radio name=opt value='unsticky'>{$lna[331]}
      <br>
      <input type=radio name=opt value='changeauthor'>{$lna[873]}
      {$adminselection}
      <input type=radio name=opt value='move'>{$lna[250]}
      <select name="newcategory">
        $puttingcates
      </select>
      <input type=radio name=opt value='newppt'>{$lna[332]}
      <select name="newproperty">
        <option value=0>{$lna[269]}</option>
        <option value=1>{$lna[270]}</option>
        <option value=2>{$lna[271]}</option>
      </select>
      <br>
      <input type=button value="{$lna[64]}" class='formbutton' onclick="adminSubmitAjax('f_s');">
    </td>
  </tr>
</table>
</form>
...
<div align=center width=70%>{$lna[333]}
  <img src='admin/theme/{$themename}/openblog.gif'>{$lna[269]}
  <img src='admin/theme/{$themename}/lockblog.gif'>{$lna[270]}
  <img src='admin/theme/{$themename}/secretblog.gif'>{$lna[271]}
</div>
...
if ($job == 'batch') {
    ...
    if (($opt == 'del' || $opt == 'noreply') && $confirm != 1) {
        $convey .= "
<form action='admin.php?go=entry_batch' method='post'>{$lna[335]}
...
<div align=center>
  ...
  <input type=button onclick='window.location=(\"admin.php?act=entry\");' value='{$lna[138]}' class='formbutton'>
    ...
    if ($opt == 'del' || $opt == 'deldraft') {
        ...
        $queryact = "UPDATE `{$db_prefix}blogs` SET `category`='{$newcategory}' WHERE {$batch_id}";
    } elseif ($opt == 'sticky') {
        $queryact = "UPDATE `{$db_prefix}blogs` SET `sticky`='1' WHERE {$batch_id}";
    } elseif ($opt == 'unsticky') {
        $queryact = "UPDATE `{$db_prefix}blogs` SET `sticky`='0' WHERE {$batch_id}";
    } elseif ($opt == 'newppt') {
        $queryact = "UPDATE `{$db_prefix}blogs` SET `property`='{$newproperty}' WHERE {$batch_id}";
    } elseif ($opt == 'noreply') {
        $queryact = "DELETE FROM `{$db_prefix}replies` WHERE {$batch_id}";
    } elseif ($opt == 'publish') {
        $queryact = "UPDATE `{$db_prefix}blogs` SET `property`='0' WHERE {$batch_id}";
    } elseif ($opt == 'changeauthor') {
        $queryact = "UPDATE `{$db_prefix}blogs` SET `authorid`='{$chnadm}' WHERE {$batch_id}";
    }
    $blog->query($queryact);
    if ($opt == 'del' || $opt == 'deldraft') {
    ...
    if ($opt == 'del' || $opt == 'deldraft' || $opt == 'move') {
        recache_categories();
...
if ($job == 'deleteblog' || $job == 'deletedraft') {
    ...
    if ($returnurl) {
        $backtodefault = "{$lna[344]}|{$returnurl}";
    }
    if ($itemid == '') {
        catcherror($lna[337]);
    } else {
...
if ($job == 'draft') {
  ...
  <tr class='visibleitem'>
    <td colspan='9' align='center' height='100'>{$lna[338]}</td>
  </tr>
";
    } else {
        for ($i = 0; $i < count($detail_array); $i++) {
            ...
            if ($detail_array[$i]['property'] == 4) {
                $detail_array[$i]['title'] .= " [<b>{$lna[1174]}</b>]";
            }
            $tablebody .= "
  <tr class='$addclass'>
    <td align='center'>
    ...
    <td align='center'>{$categories[$tmp_gp]['catename']}</td>
    <td align='center'>
      <a href='javascript: ensuredel(\"{$detail_array[$i]['blogid']}\", \"0\");'>
        <img src='admin/theme/{$themename}/del.gif' alt='{$lna[78]}' title='{$lna[78]}' border='0'>
      </a>
    </td>
    <td align='center'>
      <a href='admin.php?go=edit_edit_{$detail_array[$i]['blogid']}'>
        <img src='admin/theme/{$themename}/edit.gif' alt='{$lna[77]}' title='{$lna[77]}' border='0'>
      </a>
    </td>
    <td align='center'>
      <a href='admin.php?go=entry_publish_{$detail_array[$i]['blogid']}' onclick=\"if(shutajax==0) {window.location='admin.php?go=entry_publish_{$detail_array[$i]['blogid']}&ajax=on';return false;}\">
        <img src='admin/theme/{$themename}/openblog.gif' alt='{$lna[340]}' title='{$lna[340]}' border='0'>
      </a>
    </td>
    </tr>
";
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  <tr align=center class="admintitle">
    <td width=35 align=center>{$lna[245]}</td>
    <td width=35 align=center>{$lna[297]}</td>
    <td align=center>{$lna[284]}</td>
    <td width=200 align=center>{$lna[288]}</td>
    <td width=80 align=center>{$lna[285]}</td>
    <td width=35 align=center>{$lna[78]}</td>
    <td width=35 align=center>{$lna[77]}</td>
    <td width=35 align=center>{$lna[340]}</td>
  </tr>
  {$tablebody}
eot;
  ...
  <tr class="adminoption">
    <td colspan=7>{$lna[249]}
      <input type=radio name=opt value='del'>{$lna[78]}
      <input type=radio name=opt value='publish'>{$lna[340]}
      <input type=button value="{$lna[64]}" class='formbutton' onclick="adminSubmitAjax('f_s');">
    </td>
  </tr>
eot;
...
if ($job == 'ae') {
    ...
    $previouspage = ($_SERVER['HTTP_REFERER'] == '' || strstr($_SERVER['HTTP_REFERER'],
            'login.php')) ? "index.php" : $_SERVER['HTTP_REFERER'];
