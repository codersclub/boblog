...
if ($job=='' || $job=="default") {
	...
	acceptrequest('category');
	$queryplus=($category==="") ? '' : "AND `category`='{$category}'";
	$start_id=($page-1)*$adminitemperpage;
	$detail_array=$blog->getgroupbyquery("SELECT * FROM `{$db_prefix}blogs` WHERE `property`<>3 {$queryplus}  ORDER BY `pubtime` DESC LIMIT $start_id, $adminitemperpage");
	...
	$numenries=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}blogs` WHERE `property`<>3 {$queryplus}");
	$pagebar=gen_page ($page, 5, "admin.php?go=entry_default&category={$category}", $numenries, $adminitemperpage);
...
<tr><td colspan=7>
<select name="category"><option value=''>{$lna[328]}</option>$puttingcates</select> <input type=submit value="{$lna[244]}"></td></form></tr>
...
<br>
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
<tr><td class='sect'>{$lna[334]}</td></tr></table>
eot;
}
...
if ($job=='batch') {
	...
	if (($opt=='del' || $opt=='noreply')&&$confirm!=1) {
		...
		$convey.="<br/><br/><div align=center><input type=submit value='{$lna[64]}'> <input type=button onclick='window.location=(\"admin.php?act=entry\");' value='{$lna[138]}'></div>";
		$convery.="</form>";
		$m_b=new template;
	...
	if ($opt=='del' || $opt=='deldraft') {
		$tags_affected_raw=$blog->getarraybyquery("SELECT blogid, tags FROM `{$db_prefix}blogs` WHERE {$batch_id} HAVING tags<>'' AND tags<>'>'");
...
if ($job=='draft') {
	$detail_array=$blog->getgroupbyquery("SELECT * FROM `{$db_prefix}blogs` WHERE `property`=3  ORDER BY `pubtime` DESC");
	...
	for ($i=0; $i<count($detail_array); $i++) {
		$tmp_gp=$detail_array[$i]['category'];
		$tmp_tm=gmdate('Y/m/d H:i', $detail_array[$i]['pubtime']+3600*$config['timezone']);
		$hiddensign="<img src='admin/theme/{$themename}/draft.gif' alt='' title='{$lna[339]}'>";
...
<tr><td colspan=7 height=20></td></tr>
<tr class="adminoption"><td colspan=7>{$lna[249]}{$lna[249]}<input type=radio name=opt value='del'>{$lna[78]} <input type=radio name=opt value='publish'>{$lna[340]} <input type=submit value="{$lna[64]}">
</td></tr>
eot;
...
if ($job=='pagewrite' || $job=='pagemanage') {
	$display_overall.="Not finished";
}
