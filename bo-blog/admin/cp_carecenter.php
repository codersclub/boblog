...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2 class="sectbar" align=center>
      <input type='button' value="{$lna[64]}" class='formbutton' onclick="adminSubmitAjax('f_s');"> <input type=reset value="{$lna[65]}" class='formbutton'>
    </td>
  </tr>
</table>
</form>
  ...
  <tr class='sect'>
    <td colspan=2>
      ...
      <table width=80% align=center cellpadding=4>
        ...
        <tr>
          <td colspan=4>
            <a href="#unexist" onclick="checkallbox('ajaxForm1', 'checked');">{$lna[247]}</a>
            |
            <a href="#unexist" onclick="checkallbox('ajaxForm1', '');">{$lna[248]}</a>
          </td>
        </tr>
      </table>
    </td>
...
if ($job == 'rebuildcalendar') {
    ...
    //Re-Build Calendar
    $sql = "SELECT `blogid`,`pubtime` FROM `{$db_prefix}blogs` WHERE `property`<>2 AND `property`<>3";
    $all = $blog->getgroupbyquery($sql);
    if (is_array($all)) {
        foreach ($all as $item) {
            $time = $item['pubtime'];
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  <tr align=center class="admintitle">
    ...
    <td align=center>{$lna[792]}</td>
    <td align=center width=260>{$lna[793]}</td>
    <td align=center>{$lna[78]}</td>
    <td align=center>{$lna[794]}</td>
    <td align=center>{$lna[875]}</td>
  </tr>
  {$tablebody}
  <tr>
    <td colspan=7>
  ...
  <tr class="adminoption">
    <td colspan=7 align=center>
      <input type=checkbox name=opt value='del'>{$lna[795]}
...
if ($job == 'delupload') {
    ...
    if ($filename !== '') {
        ...
        if (file_exists($detail_array['filepath'])) {
            $result = @unlink($detail_array['filepath']);
        } else {
            $result = true;
        }
        if ($result) {
...
if ($job == 'updateattachment') {
  ...
  <tr>
    ...
    <td width=60%><input type=\"file\" name='newupfile' size=35></td>
  </tr>
  <tr>
...
if ($job == 'replaceattachment') {
    ...
    if ($upload_file_size > $permission['MaxSize'] * 1024) {
        catacherror("{$lna[421]} ( {$permission['MaxSize']} KB)");
    }
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr class='sect'>
    <td colspan=2>
      {$lna[799]} {$db_prefix} {$lna[800]}
      <br>
      <font color=red>{$lna[801]}</font>
      <br>
...
<br>
<div align=center>
  <input type=button onclick="window.location='admin.php?go=carecenter_mysql';" value="{$lna[344]}">
</div>
eot;
}
...
<table class='tablewidth' align=center cellpadding=8 cellspacing=0>
  ...
  <tr>
    <td class="visibleitem">
      <table class='tablewidth' align=center cellpadding=0 cellspacing=0>
        <tr>
          <td>
            <input type="radio" name="extype" value="xml" checked onclick="swapdiv(1);"><b>{$lna[820]}</b>
          </td>
          <td>{$lna[821]}</td>
        </tr>
        <tr>
          <td>
            <input type="radio" name="extype" value="rss" onclick="swapdiv(2);"><b>RSS 2.0</b>
          </td>
          <td>{$lna[822]}</td>
        </tr>
      </table>
  ...
  <tr>
    <td class="visibleitem">
      {$lna[825]} <input type=text name='exrange' value='50' size='4'> {$lna[539]}
      <br>
      {$lna[826]} <input type=text name='expause' value='2' size='2'> {$lna[296]}
      <br>
      {$lna[827]}
      <input type=radio name='iszip' value=1 checked> {$lna[828]}
      <input type=radio  name='iszip' value=0>{$lna[829]}
      <br>
  ...
  <tr>
    <td colspan=2 class="sect">
      ...
      <div id="backupshowarea">
        <div align=center>
          <a href="admin.php?ajax=on&go=carecenter_{$gotype}">{$lna[838]}</a>
        </div>
      </div>
    </td>
  </tr>
...
eot;
    ...
    if ($current_table == 'textfile') {
        ...
        if (is_array($data_back)) {
            ...
            if ($iszip == 0) {
                writetofile("bak/" . $filename, $content_out);
            } else {
                $filename .= '.gz';
    ...
    $display_overall = str_replace("</head>",
        "\r\n<meta http-equiv=\"refresh\" content=\"{$expause};URL=admin.php?go=carecenter_xmlbackup\"></head>",
        $display_overall);
    $display_overall .= highlightadminitems('export', 'carecenter');
    $display_overall .= <<<eot
<script>
...
eot;
    ...
    $display_overall = str_replace("</head>",
        "\r\n<meta http-equiv=\"refresh\" content=\"{$expause};URL=admin.php?go=carecenter_rssbackup\"></head>",
        $display_overall);
    $display_overall .= highlightadminitems('export', 'carecenter');
    $display_overall .= <<<eot
<script>
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2 class="sect">
      ...
      <br>
      <div align=center>
        <a href="admin.php?go=carecenter_rollback">{$lna[858]}</a>
      </div>
    </td>
  </tr>
</table>
eot;
        ...
        if ($ext != '.gz' && $ext != '.xml' && $ext != '.txt') {
            catcherror($lna[861]);
        }
        if ($ext == '.gz' && !function_exists('gzopen')) {
            catcherror($lna[857]);
        }
        if (!move_uploaded_file($upload_file, "bak/{$upload_filename}")) {
            catcherror($lna[130] . 'bak/');
        }
        $display_overall .= highlightadminitems('import', 'carecenter');
        $display_overall .= <<<eot
<script>
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2 class="sect">
      ...
      {$lna[836]}
      <br>
      {$lna[862]} {$categories[$targetcate]['catename']}
      <br>
      {$lna[837]}
      <br>
      <div align=center>
        <a href="admin.php?go=carecenter_rssrollback">{$lna[858]}</a>
        |
        <a href="admin.php?go=carecenter_import">{$lna[863]}</a>
      </div>
    </td>
  </tr>
</table>
    ...
    if (file_exists("bak/{$current_file}")) {
        if (!strstr($current_file, 'textfile_')) {
            if (strstr($current_file, '.gz')) {
                ...
                $raw_data = file("bak/{$current_file}");
            }
            ...
            if ($additional['counter']) {
                $jssetcookie = "setCookie ('changevisittotal', '{$additional['counter']}', dateObjexp, null, null, false);\n";
            }
        }
    }
    $jssetcookie .= "setCookie ('endnumber', '" . ($endnumber + 1) . "', dateObjexp, null, null, false);";
    $display_overall = str_replace("</head>",
        "\r\n<meta http-equiv=\"refresh\" content=\"{$impause};URL=admin.php?go=carecenter_rollback\"></head>",
        $display_overall);
    $display_overall .= highlightadminitems('import', 'carecenter');
    $display_overall .= <<<eot
<script>
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2 class="sect">
      ...
      <br>
      <div align=center>
        <a href="admin.php?go=carecenter_recache">{$lna[866]}</a>
      </div>
    </td>
  </tr>
</table>
...
function rollback_xml($data_array)
{
    ...
    if (is_array($deletenow) && $uniquekey != '') {
        $sql_query = "DELETE FROM `{$db_prefix}{$table_name}` WHERE `{$uniquekey}` IN (" . @implode(',',
                $deletenow) . ")";
        $blog->query($sql_query);
    }
...
{
    $count_items = preg_match("/<title>(.+?)<\/title>(.+?)<pubDate>(.+?)<\/pubDate>(.+?)<description>(.+?)<\/description>/is",
        $xml, $array_match);
    if ($count_items != 0) {
        $title = addslashes($array_match[1]);
        $time = strtotime($array_match[3]);
        if (preg_match("/<content:encoded>(.+?)<\/content:encoded>/is", $xml, $array_match_possible) != 0) {
            $content = addslashes($array_match_possible[1]);
        } else {
