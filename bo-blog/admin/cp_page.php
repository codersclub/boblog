...
if ($job == 'editpage' || $job == 'restore') {
    if ($id == "") {
        ...
        $records['pagecontent'] = preg_replace_callback("/\[php\](.+?)\[\/php\]/is", "phpcode4",
            $records['pagecontent']);
    }
}
...
if ($job == 'pagewrite') { //Initialize Add only items
    ...
    $createlinks = "
  <tr bgcolor=\"#ffffff\" align=left class=\"visibleitem\" valign=top>
    <td width=100 align=center>
      {$lna[1125]}
    </td>
    <td>
      <input type=checkbox name=addshortcut value=1>{$lna[1124]}
      <br>
      {$lna[97]}
      <input type='radio' name='shortcuttarget' value='' checked>{$lna[98]} 
      <input type='radio' name='shortcuttarget' value='_blank'>{$lna[99]}
      <br>
      {$lna[95]} 
      <input type=text name='shortcutname' value='' size='50'> {$lna[102]}
      <br>
      {$lna[1126]}
    </td>
  </tr>\n";
...
if ($job == 'pagewrite' || $job == 'editpage') { //Initialize public items
    ...
    $puttingsidebar = autoradio('checkbox', 'closesidebar', array($lna[1127]), array(1),
        array(@$records['closesidebar']), array(''));
...
<form name='editentry' id='editentry' action='admin.php' method='post' enctype='multipart/form-data' {$submitjs}>
{$hiddenareas}
    ...
    <td colspan=2  class="sect">
      ...
      <table width=100% cellpadding=4 cellspacing=1 align=center>
        <tr bgcolor="#ffffff" align=left class="visibleitem">
          ...
          <td>{$puttingeditors}
            <input type=button value="{$lna[64]}" onclick="changeeditor();">
          </td>
        </tr>
        <tr bgcolor="#ffffff" align=left class="hiddenitem">
          <td width=100 align=center>{$lna[284]}</td>
          <td>
            <input type='text' name='pagetitle' id='title' value="{$records['pagetitle']}" size='50'  class='formtext'>
          </td>
        </tr>
        <tr bgcolor="#ffffff" align=left class="visibleitem">
        ...
        <tr bgcolor="#ffffff" align=left valign=top class="hiddenitem">
          ...
          <td>
            <input type=text name='pagealias' value="{$records['pagealias']}" size="50"> {$lna[102]}
            <br>
            {$lna[1118]}
          </td>
        </tr>
...
if ($job == 'store' || $job == 'restore') {
    acceptrequest('pagetitle,closesidebar,html,ubb,emot,useeditor,pagealias,addshortcut,shortcuttarget,shortcutname', 0,
        'post');
    ...
    if ($job == 'store') {
        ...
        if ($addshortcut == 1) {
            ...
            $infotoadd = "\$blogitem['pageshortcut{$currentid}']=array('type'=>'link', 'name'=>'pageshortcut{$currentid}', 'url'=>'" . getlink_pages($currentid,
                    $pagealias) . "', 'text'=>'{$shortcuttext}' {$shortcuttarget});";
