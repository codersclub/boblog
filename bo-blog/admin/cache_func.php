...
if (!defined('VALIDADMIN') && !defined('REPLYSPECIAL')) {
    die ('Access Denied');
}
...
function recache_categories()
{
    ...
    if (!writetofile("data/cache_categories.php", $writeout)) {
        catcherror($lna[66] . "data/cache_categories.php");
    }
}
...
function recache_latestreplies()
{
    ...
    if (!writetofile("data/cache_replies.php", $outcache_limit)) {
        catcherror($lna[66] . "data/cache_replies.php");
    }
}
...
function recache_taglist()
{
    ...
    if (!writetofile("data/cache_tags.php", $out)) {
        catcherror($lna[66] . "data/cache_tags.php");
    }
}
...
function recache_cleartemp()
{
    ...
    if (!$handle) {
        catcherror("{$lna[155]} {$db_tmpdir}/ {$lna[156]}<ul><li>{$lna[157]}</li><li>{$lna[158]}</li><li>{$lna[159]}</li></ul>");
    }
