...
if ($job == 'censor' || $job == 'default') {
    ...
    for ($i = 0; $i < count($detail_array); $i++) {
        ...
        $tablebody .= "<tr class='visibleitem'><td align='center'><input type='checkbox' name='selid[]' id='selid[]' value='{$detail_array[$i]['repid']}-{$detail_array[$i]['blogid']}'></td><td>{$detail_array[$i]['replier']}</td><td>{$tmp_tm}</td><td align='left' width=50%><a href='" . getlink_entry($detail_array[$i]['blogid'],
  ...
  <td align='center'>
    <a href='javascript: redirectcomfirm(\"admin.php?go=reply_{$tbactdel}_{$detail_array[$i]['repid']}-{$detail_array[$i]['blogid']}\");'>
      <img src='admin/theme/{$themename}/del.gif' alt='{$lna[78]}' title='{$lna[78]}' border='0'>
        ...
        if ($job == 'tbcensor') {
            $tablebody .= "  <td align='center'><a href=\"javascript: simulateFormSubmit('admin.php?go=reply_tbpass_{$detail_array[$i]['repid']}-{$detail_array[$i]['blogid']}')\"><img src='admin/theme/{$themename}/yes.gif' alt='{$lna[259]}' title='{$lna[259]}' border='0'></a></td>";
        }
        $tablebody .= "  </tr>\n";
    }
