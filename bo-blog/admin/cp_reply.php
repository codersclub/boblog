...
if ($job == 'delreply') {
    ...
    if (!$returnurl) {
        $returnurl = "admin.php?go=reply_default";
    }
    if (!is_array($repid)) {
        $tmp_array[0] = $repid;
        $repid = $tmp_array;
...
if ($job == 'censor' || $job == 'default') {
    ...
    if ($job == 'censor') {
        ...
        $censorclearall = "<br>\n<br>\n[<a href=\"javascript: redirectcomfirm('admin.php?go=reply_repliesclearall');\">{$lna[1021]}</a>]\n";
    } elseif ($job == 'default') {
  ...
  <tr class='visibleitem'>
    ...
    <td align='left' width=50%>
      <a href='" . getlink_entry($detail_array[$i]['blogid'], '')
            . "' target='_blank' title='{$lna[356]}'>{$detail_array[$i]['repcontent']}</a>
    </td>
    <td align='center'>
      <a href='javascript: ensuredel(\"{$detail_array[$i]['repid']}-{$detail_array[$i]['blogid']}\", \"{$param2}\");' title='{$lna[78]}'>
        <img src='admin/theme/{$themename}/del.gif' alt='{$lna[78]}' border='0'>
      </a>
    </td>
    <td align='center'>
      <a href=\"javascript: simulateFormSubmit('admin.php?go=reply_{$address}_{$detail_array[$i]['repid']}-{$detail_array[$i]['blogid']}')\"
         title='$titles'>
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  ...
  <tr>
    ...
    <td colspan=3 align=right>{$pagebar}</td>
  </tr>
  <tr>
  ...
  <tr class="adminoption">
    <td colspan=7>
      {$lna[249]}
      <input type=radio name='job' value='delreply'>{$lna[78]}
      <input type=radio name='job' value='{$address}'>{$titles}
      <input type=button value="{$lna[64]}" class='formbutton' onclick="adminSubmitAjax('f_s');">{$censorclearall}
    </td>
  </tr>
</table>
</form>
...
<tr class='visibleitem'>
  ...
  <td align='center'>
    <a href='javascript: redirectcomfirm(\"admin.php?go=reply_{$tbactdel}_{$detail_array[$i]['repid']}-{$detail_array[$i]['blogid']}\");'
       title='{$lna[78]}'>
      <img src='admin/theme/{$themename}/del.gif' alt='{$lna[78]}' border='0'>
    </a>
  </td>\n";
        ...
        <a href=\"javascript: simulateFormSubmit('admin.php?go=reply_tbpass_{$detail_array[$i]['repid']}-{$detail_array[$i]['blogid']}')\"
           title='{$lna[259]}'>
          <img src='admin/theme/{$themename}/yes.gif' alt='{$lna[259]}' border='0'>
        </a>
      </td>\n";
        }
        $tablebody .= "  </tr>\n";
    ...
    if ($job == 'tbcensor') {
        ...
        $censorclearall = "<br>\n<br>\n[<a href=\"javascript: redirectcomfirm('admin.php?go=reply_tbclearall');\">{$lna[1021]}</a>]\n";
        $titlem = $lna[947];
        $titler = $lna[948];
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  ...
  <tr class="adminoption">
    <td colspan=7>{$lna[249]}
      <input type=radio name='job' value='{$tbactdel}'>{$lna[78]}
      {$censorplus2}
      <input type=button value="{$lna[64]}" class='formbutton' onclick="adminSubmitAjax('f_s');">{$censorclearall}
    </td>
  </tr>
