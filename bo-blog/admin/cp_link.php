...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
    ...
    <td colspan=2  class="sect">
      <table class='tablewidth' align=center>
        <tr>
          ...
          <td>
            <input type="text" name="newlinkgpname" value="" size="30">
          </td>
          <td>{$lna[218]}</td>
          <td>
            <select name="newlinkgppt">
              <option value="1">{$lna[219]}</option>
              <option value="0">{$lna[220]}</option>
            </select>
          </td>
        </tr>
        <tr>
  ...
  <tr>
    <td colspan=4 align=center class="sectbar">
      <input type=button value="{$lna[64]}" class='formbutton' onclick="clearAllRadios();adminSubmitAjax(1);">
    ...
    <td colspan=2  class="sect">
      <table class='tablewidth' align=center>
        <tr>
          ...
          <td width=50%>
            {$lna[200]}
            <input type=radio name='go' value='link_editgp' onclick="swapdiv(0);">{$lna[77]}
            <input type=radio name='go' value='link_order' onclick="swapdiv(0);">{$lna[227]}
            <input type=radio  name='go' value='link_delgp' onclick="swapdiv(1);">{$lna[78]}
            <input type=radio  name='go' value='link_combinegp' onclick="swapdiv(2);">{$lna[201]}
          </td>
        </tr>
        <tr>
          <td width=50%>&nbsp;</td>
          <td width=50%>
            <div id="targetdiv3" style="display:none">
              <input type=submit value="{$lna[64]}" class='formbutton'>
            </div>
            <div id="targetdiv" style="display:none">
              {$lna[228]}
              <select name="targetlinkgp2">
                $puttinggp2
              </select>
    ...
    <td colspan=2  class="sect">
      ...
      <table class='tablewidth' align=center cellpadding=4 cellspacing=0>
        <tr>
          ...
          <td>
            <input type="text" name="newlinkgpname" value="{$linkgps[$sourcelinkgp]['linkgpname']}">
          </td>
        </tr>
        <tr>
          ...
          <td>
            <select name="newlinkgppt">
              <option value="1" {$sel1}>{$lna[219]}</option>
              <option value="0" 	{$sel2}>{$lna[220]}</option>
            </select>
          </td>
        </tr>
      </table>
    ...
    <td colspan=2  class="sect">
      ...
      <table class='tablewidth' align=center cellpadding=4 cellspacing=0>
        <tr>
          ...
          <td>
            <input type="text" name="newlinkname" value="{$linkvalue['linkname']}">
          </td>
        </tr>
        <tr>
          <td width=160>{$lna[234]}</td>
          <td>
            <input type="text" name="newlinkurl" value="{$linkvalue['linkurl']}">
          </td>
        </tr>
        <tr>
          <td width=160>{$lna[235]}</td>
          <td>
            <input type="text" name="newlinklogo" value="{$linkvalue['linklogo']}">
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            {$lna[236]}
          </td>
        </tr>
        <tr>
          ...
          <td>
            <input type="text" name="newlinkdesc" value="{$linkvalue['linkdesc']}">
          </td>
        </tr>
        <tr>
          <td width=160>{$lna[237]}</td>
          <td>
            <select name="newlinkgptoid">
              {$puttinggp3}
            </select>
          </td>
        </tr>
        <tr>
          ...
          <td>
            <select name="newisdisplay">
	          <option value='1' $sel1>{$lna[239]}</option>
	          <option value='0' $sel2>{$lna[240]}</option>
	        </select>
            <input type=hidden name=go value="link_save">
            {$addhidden}
          </td>
        </tr>
      </table>
...
if ($job == 'detail') {
    ...
    for ($i = 0; $i < count($detail_array); $i++) {
        ...
        if ($linkgps[$tmp_gp]['linkgppt'] == 0 || $detail_array[$i]['isdisplay'] == 0) {
            $addclass = 'hiddenitem';
        } else {
            $addclass = 'visibleitem';
        }
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  <tr>
    <td colspan=7>
      <select name="linkgptoid">
        <option value=''>{$lna[243]}</option>
        $puttinggp
      </select>
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  ...
  <tr align=center class="admintitle">
    <td width=35>{$lna[245]}</td>
    <td width=95>Logo</td>
    <td width=220>{$lna[233]}</td>
    <td width=250>{$lna[234]}</td>
    <td width=80>{$lna[237]}</td>
    <td width=35>{$lna[246]}</td>
    <td width=35>{$lna[77]}</td>
  </tr>
  {$tablebody}
  <tr>
    <td colspan=7>
  ...
  <tr class="adminoption">
    <td colspan=7>
      ...
      <input type=radio name=opt value='ppt'>{$lna[238]}
      <select name="newproperty">
        <option value=1>{$lna[239]}</option>
        <option value=0>{$lna[240]}</option>
      </select>
...
if ($job == 'batch') {
    ...
    catcherror($cancel);
    if ($opt == 'del') {
        $queryact = "DELETE FROM `{$db_prefix}links` WHERE `linkid`='::'";
    } elseif ($opt == 'move') {
        $queryact = "UPDATE `{$db_prefix}links` SET `linkgptoid`='{$newlinkgptoid}' WHERE `linkid`='::'";
    } else {
        $queryact = "UPDATE `{$db_prefix}links` SET `isdisplay`='{$newproperty}' WHERE `linkid`='::'";
    }
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2 class="sect">
      {$lna[231]} {$linkgps[$linkgptoid]['linkgpname']}
      <br>
...
if ($job == "pending") {
    ...
    $totoshow = 0;
    if (!file_exists($filename)) {
        $tablebody = "<tr><td colspan=10 align=center><br><br>{$lna[254]}<br><br></td></tr>";
    } else {
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  <tr class="admintitle">
    ...
    <td width=150 align=center>{$lna[233]}</td>
    <td width=250 align=center>{$lna[234]}</td>
    <td width=35 align=center>Logo</td>
    <td align=center>{$lna[134]}</td>
    <td width=30 align=center>{$lna[259]}</td>
    <td width=30 align=center>{$lna[260]}</td>
  </tr>
  $tablebody
eot;
  ...
  <tr class="adminoption">
    <td colspan=7>
	  ...
	  <input type=radio name=opt value='del'>{$lna[78]}
	  <br>
	  <input type=radio name=opt value='accept'>{$lna[261]}
	  <select name="newlinkgptoid">
        $puttinggp
      </select>
	  <br>
	  <input type=radio name=opt value='textonly'>{$lna[262]}
	  <select name="newlinkgptoid2">
        $puttinggp
      </select>
      <br>
	  <div align=center>
