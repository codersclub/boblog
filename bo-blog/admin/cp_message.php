...
/*vot*/		$countreps=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}messages` WHERE `reproperty`<>2");
...
if ($job=='censor' || $job=='default') {
	...
	if ($job=='censor') {
		...
		$titler=$lna[347];
		$param2=5;
/*vot*/		$totalvolume=$blog->countbyquery("SELECT COUNT(*) FROM `{$db_prefix}messages` WHERE `reproperty`=2 OR `reproperty`=3");
