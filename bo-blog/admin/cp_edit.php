...
if ($job != 'add' && $job != 'store' && $job != 'sendtb') {
    if ($id == "") {
        ...
        $records['entrysummary'] = preg_replace_callback("/\[php\](.+?)\[\/php\]/is", "phpcode4",
            $records['entrysummary']);
...
if ($job == 'add' || $job == 'edit') { //Initialize public items
    ...
    if (@$flset['weather'] != 1 && is_array($weather)) { //Get Weather List
        foreach ($weather as $wkey => $wvalue) {
...
if ($job == 'edit') {                                                                  //Initialize Edit only items
    if (@$flset['weather'] != 1) {
        $selectedid_weather = array_search($records['weather'], $arrayvalue_weather);
    }                                                                                  //selected weather
    ...
    if ($permission['AddTag'] != 1) {
        $taglist .= "<input type='hidden' name='tags' value='{$records['tags']}'>";
    }
    if ($records['permitgp'] !== '') {
        $allowedgp = @explode('|', $records['permitgp']);
        foreach ($usergp as $gpid => $gpname) {
            if (@!in_array($gpid, $allowedgp)) {
                $arraychecked_permitgp[] = 1;
            } else {
                $arraychecked_permitgp[] = 0;
            }
        }
    }
    ...
    if ($records['entrysummary']) {
        $entrysummaryplus1 = " selected";
    } elseif ($records['frontpage'] == 1) {
        $entrysummaryplus2 = " selected";
    } else {
...
if ($job == 'add' || $job == 'edit') { //Initialize public items
    @list($records['pub_year'], $records['pub_month'], $records['pub_day'], $records['pub_hour'], $records['pub_min'], $records['pub_sec']) = explode('-',
        $records['pub_tmp']);
    if ($permission['Html'] == 1) {
        $disablehtmlstatus = 0;
    } else {
    ...
    $puttingcates = str_replace('<select',
        "<select onchange=\"if (this.options[this.selectedIndex].value=='new') {document.getElementById('addnewcate').style.display='block';} else {document.getElementById('addnewcate').style.display='none';}\"  ",
        $puttingcates);
    ...
    $records['content'] = preg_replace_callback("/\[php\](.+?)\[\/php\]/is", "phpcode4", @$records['content']);
    ...
    <td colspan=2  class="sect">
      ...
      <table width=100% cellpadding=4 cellspacing=1 align=center>
        <tr bgcolor="#ffffff" align=left class="hiddenitem" id="extraoption1">
          <td width=100 align=center>{$lna[567]}</td>
          <td>
            {$puttingeditors} <input type=button value="{$lna[64]}" onclick="changeeditor();">
          </td>
        </tr>
        <tr bgcolor="#ffffff" align=left class="visibleitem">
          <td width=100 align=center>{$lna[284]}</td>
          <td>
            <input type='text' name='title' id='title' value="{$records['title']}" size='50'  class='formtext'>
          </td>
        </tr>
        <tr bgcolor="#ffffff" align=left class="hiddenitem">
          <td width=100 align=center valign=top>{$lna[285]}</td>
          <td>
            <div id='cateselarea'>
              {$puttingcates} {$lna[286]}

              <div id='addnewcate' style='display: none;'>
                <table width='95%' align=left>
                  <tr>
                    <td width=100>{$lna[182]}</td>
                    <td>
                      <input type="text" name="newcatename" id="newcatename" value="" size="30">
                    </td>
                  </tr>
                  <tr>
                    <td width=100 valign=top>{$lna[183]}</td>
                    <td>
                      <textarea cols=30 rows=4 name="newcatedesc" id="newcatedesc"></textarea>
                    </td>
                  </tr>
                  <tr>
                    <td width=100>{$lna[187]}</td>
                    <td>
                      <select name="newcatemode" id="newcatemode">
                        <option value="0">{$lna[188]}</option>
                        <option value="1">{$lna[189]}</option>
                      </select>
                    </td>
                  </tr>
                  <tr>
                    <td width=100>{$lna[184]}</td>
                    <td>
                      <select name="newcateproperty" id="newcateproperty">
                        <option value="0">{$lna[185]}</option>
                        <option value="1">{$lna[186]}</option>
                      </select>
                    </td>
                  </tr>
                  <tr>
                    <td width=100>{$lna[1022]}</td>
                    <td>
                      <select name="targetcate" id="targetcate">
                        <option value='-1'>{$lna[1023]}</option>
                        <option value='-2' selected>{$lna[1024]}</option>
                        $puttingcates_after
                      </select>
                    </td>
                  </tr>
                  <tr>
                    <td colspan=2>
                      <input type=button value="{$lna[64]}" onclick="ajax_addcategory();">
                      <input type=button value="{$lna[138]}" onclick="showhidediv('addnewcate');">
                    </td>
                  </tr>
                </table>
              </div>
...
eot;
    ...
    $display_overall .= <<<eot
        ...
        <tr bgcolor="#ffffff" align=left class="hiddenitem" id="extraoption2">
          ...
          <td>
            <select name='summaryway' id='summaryway' onchange="if (this.options[this.selectedIndex].value=='1') {document.getElementById('entrysummary').style.display='block';} else {document.getElementById('entrysummary').style.display='none';}">
              <option value="0"{$entrysummaryplus0}>{$lna[1113]}</option>
              <option value="2"{$entrysummaryplus2}>{$lna[1114]}</option>
              <option value="1"{$entrysummaryplus1}>{$lna[1115]}</option>
            </select>
        ...
        <tr bgcolor="#ffffff" align=left valign=top class="visibleitem">
          <td width=100 align=center>{$lna[1117]}</td>
          <td>
            <input type=text name='blogalias' id='blogalias' value="{$records['blogalias']}" size="50"> {$lna[102]}
            <br>
            {$lna[1118]}
          </td>
        </tr>
        <tr bgcolor="#ffffff" align=left valign=top class="hiddenitem" id="extraoption3">
          <td width=100 align=center>{$lna[1119]}</td>
          <td>
            {$lna[1120]}: <input type=text name='originsrc' id='originsrc' value="{$records['originsrc']}" size="50">{$lna[1121]}
            &nbsp;&nbsp; {$lna[1122]}: <input name='comefrom' id='comefrom' type=text value="{$records['comefrom']}" size="20">
          </td>
        </tr>
        ...
        <tr bgcolor="#ffffff" align=left class="hiddenitem" id="extraoption5">
          ...
          <td>
            {$lna[298]} {$puttingproperty} ({$lna[299]})
            <br>
            {$lna[300]} {$puttingsticky}
            <br>
            {$lna[301]} {$puttingweather}
          </td>
        </tr>
...
eot;
    ...
    $display_overall .= <<<eot
        <tr bgcolor="#ffffff" align=left class="visibleitem" id="extraoption7">
          ...
          <td>
            {$resendping}
            <textarea name='pinged' id='pinged' rows='2' cols='100' class='formtextarea'>{$records['pinged']}</textarea>
            <br>
            {$lna[306]}
          </td>
        </tr>
        <tr bgcolor="#ffffff" align=left class="hiddenitem" id="extraoption8">
          <td width=100 align=center valign=top>{$lna[1080]}</td>
          <td>
            <input type='text' name='blogpsw' id='blogpsw' value="{$records['blogpsw']}" size='15' maxlength='18' class='formtext'> {$lna[1081]}
          </td>
        </tr>
      </table>
  ...
  <tr>
    <td colspan=4 align=center class="sectbar">
      <input type=button value="{$lna[64]}" onclick="chktitle();" class="formbutton">
...
<div style='visibility: hidden'>
  <input type=submit value="{$lna[64]}" id='realsubmit' class='formbutton'>
</div>
</form>
...
if ($job == 'store' || $job == 'restore') {
    ...
    if ($categories[$category]['cateproperty'] == 1) {
        $property = 2;
    }
    if (($forcedraft == 1 && $property != 4) || $ajax == 'on') {
        $property = 3;
    }
    if ($forcedraft == 2 && $ajax != 'on') {
        $property = 0;
    }
    ...
    if ($summaryway == 1) {
        ...
        //If magic quotes is on, strip the slashes automatically added
        if ($mqgpc_status == 1) {
            $entrysummary = stripslashes($entrysummary);
        }
        if ($autobr == 0) {
        ...
        $entrysummary = preg_replace_callback("/\[php\](.+?)\[\/php\]/is", "phpcode3", $entrysummary);
        if ($htmlstat != 1 || $permission['Html'] != 1) {
            $entrysummary = preg_replace_callback("/\[code\](.+?)\[\/code\]/is", "phpcode2", $entrysummary);
            $entrysummary = safe_convert($entrysummary, 0, 1);
        } else {
            $entrysummary = preg_replace_callback("/\[code\](.+?)\[\/code\]/is", "phpcode", $entrysummary);
    ...
    if ($tags || $records['tags'] != '') {
        ...
        for ($m = 0; $m < count($newtags); $m++) {
            if ($newtags[$m] !== '') {
                $blog->query("INSERT INTO `{$db_prefix}tags` VALUES ('{$currenttime}', '{$newtags[$m]}', 1, '<tag>,{$currentid},<end>', '')");
            }
        }
    }
    ...
    if ($job == 'restore' && $property != $records['property']) {
    ...
    if (($job == 'store' && !$pinged) || ($job == 'restore' && $resend != 1) || ($job == 'restore' && !$pinged)) {
        ...
        $form = "
<div align=center>
  <form action='admin.php?go=edit_sendtb' method='post'>
    <input type='hidden' name='title' value=\"{$title}\">
    <input type='hidden' name='excerpt' value=\"{$excerpt}\">
    <input type='hidden' name='blog_name' value=\"{$config['blogname']}\">
    <input type='hidden' name='url' value='{$config['blogurl']}/"
            . get_entry_url($currentid, $blogalias) . "'>
    {$ping_urls}
    <input type='submit' value='{$lna[310]}' class='formbutton'>
...
function sendping($url, $title, $excerpt, $blog_url, $blog_name)
{
    ...
    if (strstr($http_content, "<error>0</error>")) {
        return ("ok");
        ...
        return (htmlspecialchars($http_content));
    }
}
