...
if ($job == 'weatherset') {
    if ($weather) {
        ...
        foreach ($weather as $weathername => $weatherdetail) {
            $formbody .= "
  <tr>
    <td colspan=2 class='prefsection'>{$lna[361]}{$weatherdetail['text']}</td>
  </tr>\n";
            if ($weathername != 'blank') {
                $formbody .= "
  <tr>
    <td class='prefleft'>{$lna[362]}</td>
    <td class='prefright'>
      <input type=text size=20 name='variable[{$i}]' value='{$weathername}'> {$lna[363]}
    </td>
  </tr>\n";
            } else {
                $formbody .= "
  <tr>
    <td class='prefleft'>{$lna[362]}</td>
    <td class='prefright'>
      <input type=text size=20 name='variable[{$i}]' value='{$weathername}' disabled>
      <input type=hidden name='variable[{$i}]' value='{$weathername}'> {$lna[364]}
    </td>
  </tr>\n";
            }
            $formbody .= "
  <tr>
    <td class='prefleft'>{$lna[365]}</td>
    <td class='prefright'>
      <input type=text size=20 name='desc[{$i}]' value='{$weatherdetail['text']}'>
    </td>
  </tr>\n";
            $formbody .= "
  <tr>
    <td class='prefleft'>{$lna[366]}</td>
    <td class='prefright'>
      <input type=text size=40 name='icon[{$i}]' value='{$weatherdetail['image']}'>
      <img src='{$weatherdetail['image']}'>
    </td>
  </tr>\n";
            if ($weathername != 'blank') {
                $formbody .= "
  <tr>
    <td class='prefleft'>{$lna[367]}</td>
    <td class='prefright'>
      <input type=button value='{$lna[368]}' onclick=\"makesuredelweather('$weathername');\" class='formbutton'>
    </td>
  </tr>\n";
            }
            $i += 1;
...
<table class='tablewidth' cellpadding=4 cellspacing=1 align=center>
  <tr>
    <td colspan=2 class='prefsection'>{$lna[370]}</td>
  </tr>
  <tr>
    <td class='prefleft'>{$lna[362]}</td>
    <td class='prefright'>
      <input type=text size=20 name='newvariable' value=''> {$lna[363]}
    </td>
  </tr>
  <tr>
    <td class='prefleft'>{$lna[365]}</td>
    <td class='prefright'>
      <input type=text size=20 name='newdesc' value=''>
    </td>
  </tr>
  <tr>
    <td class='prefleft'>{$lna[366]}</td>
    <td class='prefright'>
      <input type=text size=40 name='newicon' value=''>
    </td>
  </tr>
</table>
...
if ($job == 'weathersave') {
    ...
    if (writetofile("data/weather.php", $wholeout)) {
        if ($ajax == 'on') {
            ...
            catchsuccess($finishok, array($backtoweather, $backtoemot, $backtoforbidden, $backtoava));
        }
    } else {
...
if ($job == 'avatar') {
  ...
  <tr>
    <td colspan='2' class="sect" align='center'>
      {$lna[373]}
        <a href="admin.php?go=misc_avatar_open">{$lna[374]}</a>
    </td>
  </tr>
eot;
    }
...
if ($job == 'avatarrefresh') {
    ...
    if (!$handle) {
        catcherror("{$lna[155]} images/avatars {$lna[156]}<ul><li>{$lna[157]}</li><li>{$lna[158]}</li><li>{$lna[159]}</li></ul>");
    }
    while (false !== ($file = readdir($handle))) {
        if ($file != "." && $file != ".." && (stristr($file, '.gif') || stristr($file, '.png') || stristr($file,
                    '.jpg'))) {
            $savetext .= "'{$file}',\n";
        }
    }
    ...
    if (writetofile("data/cache_avatars.php", $saveall)) {
        if ($ajax == 'on') {
            ...
            catchsuccess($finishok, array($backtoava, $backtoemot, $backtoforbidden, $backtoweather));
        }
    } else {
...
if ($job == 'emot') {
  ...
  <tr class='sect' align=center>
    <td width=40>
      <img src='images/emot/{$emotthumb}'>
    </td>
    <td>
      <input type=text size=9 name='variable[{$i}]' value='{$emotcode}'>
    </td>
    <td>
      <input type=text size=25 name='image[{$i}]' value='{$emotimage}'>
    </td>
    <td>
      <input type=text size=25 name='thumb[{$i}]' value='{$emotthumb}'>
    </td>
  </tr>\n";
            $i += 1;
            //if ($i==25) break;
  ...
  <tr class='sect' align=center>
    <td width=40>&nbsp;</td>
    <td>
      <input type=text size=9 name='variable[{$i}]' value=''>
    </td>
    <td>
      <input type=text size=25 name='image[{$i}]' value=''>
    </td>
    <td>
      <input type=text size=25 name='thumb[{$i}]' value=''>
    </td>
  </tr>\n";
        $i += 1;
    }
...
if ($job == 'forbidden') {
    ...
    foreach ($item_name as $key => $item) {
        ...
        $selbody[$key] = "
<a name='f_{$key}'></a>
<table class='tablewidth' cellpadding=4 align=center>
  <tr>
    <td class='sect'>
      <table class='tablewidth' cellpadding=1 align=center>
        <tr>
          <td class='sect'>
            <b>{$item}</b>
            <br>
            <textarea name='{$key}' cols='80' rows='5'>{$bodyarea}</textarea>
            <br>
            {$item_desc[$key]} $lna[402]
          </td>
        </tr>
      </table>
    </td>
...
<table cellpadding=3 cellspacing=1 align=center class='tablewidth'>
  <tr>
    <td class='sect' align=center>
      <input type='button' value="{$lna[404]}" class='formbutton' onclick="adminSubmitAjax(1);">
      <input type=reset value="{$lna[405]}" class='formbutton'>
    </td>
  </tr>
</table>
</form>
  ...
  <tr>
    <td colspan=2 class='sect'>
      ...
      <table cellpadding=3 cellspacing=1 align=center width=80%>
        <tr>
          <td width=25% align=right>{$lna[408]}</td>
          <td width=5%>&nbsp;</td>
          <td width=70%>
        ...
        <tr>
          <td width=25% align=right>{$lna[410]}</td>
          <td width=5%>&nbsp;</td>
          <td width=70%>
            <select name="destination">{$option_all}</select>
          </td>
        </tr>
        <tr>
          <td class='sect' align=center colspan=3>
  ...
  <tr>
    <td colspan=2 class='sect'>
      ...
      <table cellpadding=6 cellspacing=1 align=center width=95%>
        <tr>
          <td colspan=2>{$lna[977]}</td>
        </tr>
        <tr>
          <td width=25% align=right valign=top>{$lna[482]}</td>
          <td>
        ...
        <tr>
          <td width=25% align=right valign=top>{$lna[978]}</td>
          <td>
        ...
        <tr>
          <td width=25% align=right valign=top>{$lna[824]}</td>
          <td>
            <input type=checkbox value=1 name="usedefaultsess" {$checked}>
            {$lna[1000]}
            <br>
            {$lna[1001]}
          </td>
        ...
        <tr>
          <td class='sect' align=center colspan=2>
            <input type='button' value="{$lna[64]}" class='formbutton' onclick="adminSubmitAjax(1);">
            <input type=reset value="{$lna[65]}" class='formbutton'>
          </td>
        </tr>
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2 class='sect' align=center>
      <iframe id="ifa" width=250 height=100 src='data/tmpsesstest1.php'></iframe>
      <br>
      <br>
      {$lna[1002]}
      <br>
      <input type=button onclick="document.getElementById('ifa').src='data/tmpsesstest2.php'" value='{$lna[1003]}'>
      <br>
      <br>
      {$lna[1004]}
      <br>
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
  ...
  <tr>
    <td colspan=2 class='sect'>
      <br>
      <input type=radio value='0' name="urlrewritesta" {$checksta1} onclick="document.getElementById('urloptmessagearea').innerHTML=document.getElementById('urloptmessage0').value; document.getElementById('apachearea').style.display='none'"> {$lna[511]}
      <br>
      <input type=radio value='1' name="urlrewritesta" {$checksta2} onclick="document.getElementById('urloptmessagearea').innerHTML=document.getElementById('urloptmessage1').value; document.getElementById('apachearea').style.display='none'"> {$lna[938]} ({$lna[1139]})
      <br>
      <input type=radio value='2' name="urlrewritesta" {$checksta4} onclick="document.getElementById('urloptmessagearea').innerHTML=document.getElementById('urloptmessage3').value; document.getElementById('apachearea').style.display='block'"> {$lna[1138]}
      <br>
