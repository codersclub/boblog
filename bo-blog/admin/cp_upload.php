...
if (!$job || $job == "default") {
  ...
  <tr>
    ...
    <td width=60%>
      ...
      <input type=\"file\" name='newupfile[]'>
      <br>
      {$lna[677]} {$display_allowed}
    </td>
  </tr>
  <tr>
    <td colspan=10 align=center>
      <input type='submit' value='{$lna[414]}' class='formbutton'>
...
if ($job == "doattach") {
  ...
  <br>
  <div align=center>
    <input type='button' value='{$lna[423]}' onclick='window.location=\"admin.php?go=upload&useeditor={$useeditor}\";'>
    <span style='display: none;'>
      <input type='checkbox' id='ifautoaddubb' checked='checked'>
    </span>
  </div>
</div>\n");
}
...
if ($job == "gallery") {
  ...
  <option value=0>{$lna[292]}</option>\n";
    ...
    $pagebar = gen_page($page, 3,
        "admin.php?go=upload_gallery&useeditor={$useeditor}&uploadyear={$uploadyear}&uploadmonth={$uploadmonth}",
        $numenries, 51);

    $message = "<form action='admin.php?go=upload_gallery&useeditor={$useeditor}' method=post>
...
<div>
  <div id='uploadrow'  style=\"width: 210px; float: left; overflow-y: auto; height: 165px;\">
    <ul>" . @implode("\n", $inserttext)
        . "</ul>
  </div>
  <div id='picp' style='margin-top: 0px;  width: 400px !important; width: 320px; height: 165px;'>
    {$lna[427]}
  </div>
</div>\n";
