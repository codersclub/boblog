...
if ($job == 'skin') {
    ...
    $tablebody = "
  <tr class='sect'>
    <td width=412 align=center class='sect'>
      <img src='template/{$template['dirname']}/{$template['thumbnail']}' alt='' border=0>
    </td>
    <td class='visibleitem'>
      <font color=red>
        <b>{$lna[161]}</b> {$template['name']}
      </font>
      <br>
      <b>{$lna[162]}</b> {$template['author']}
      <br>
      <b>{$lna[163]}</b> {$template['intro']}
      <br>
      <br>
    </td>
  </tr>\n";
    ...
    if (is_array($skinset)) {
        foreach ($skinset as $skin) {
            ...
            $tablebody .= "<tr class='sect'>
    <td width=412 align=center class='sect'>
      <a href=\"javascript: simulateFormSubmit('admin.php?go=addon_setskin_{$skdir}')\" title='{$lna[164]}' class='visibleitem'>
        <img src='template/{$skdir}/{$skthumbnail}' alt='{$lna[164]}' border=0>
      </a>
    </td>
    <td class='visibleitem'>
      <b>{$lna[165]}</b> {$skname}
      <br>
      <b>{$lna[162]}</b> {$skauthor}
      <br>
      <b>{$lna[163]}</b> {$skintro}
      <br>
      <br>
      <a href=\"javascript: simulateFormSubmit('admin.php?go=addon_setskin_{$skdir}')\" title='{$lna[164]}'>{$lna[166]}</a>
      |
      <a href=\"javascript: redirectcomfirm('admin.php?go=addon_removeskin_{$skid}');\" title='{$lna[167]}'>{$lna[168]}</a>
    </td>
  </tr>\n";
        }
    }
...
<table align=center class='tablewidth'>
  {$tablebody}
  <tr>
    <td colspan=2 align=right>{$pagebar}</td>
  </tr>
</table>
    ...
    <table class='tablewidth' align=center cellpadding=4 cellspacing=0>
        ...
        <tr class='sect'>
            <td colspan=2 align=center>
                <br>
                {$lna[173]} template/<input type=text name=newskindir size=20>/ <input type='button' value="{$lna[64]}" class='formbutton' onclick="adminSubmitAjax(1);"><br>
            </td>
        </tr>
    </table>
</form>
...
if ($job == 'scanskin') {
    ...
    if (!$handle) {
        catcherror("{$lna[155]} template/ {$lna[156]}
  <ul>
    <li>{$lna[157]}</li>
    <li>{$lna[158]}</li>
    <li>{$lna[159]}</li>
    ...
    while (false !== ($file = readdir($handle))) {
        if (!empty($file) && $file != "." && $file != ".." && file_exists("template/{$file}/info.php")) {
            ...
            $addline .= "\$skinset[\"{$template['id']}\"]=\"{$template['id']}"
                     . "|" . skin_convert($template['name'])
                     . "|" . skin_convert($template['author'])
                     . "|" . skin_convert($template['intro'])
                     . "|" . skin_convert($template['dirname'])
                     . "|" . skin_convert($template['thumbnail'])
                     . "|\";\n";
        }
    }
    ...
    if (!writetofile("data/cache_skinlist.php", $addline)) {
        ...
        if ($ajax == 'on') {
            ...
            catchsuccess($finishok2, $backtoskin);
        }
    }
}
...
if ($job == 'addskin') {
    ...
    if (!$newskindir || !is_dir("template/{$newskindir}")) {
        catcherror($lna[174]);
    }

    if (!file_exists("template/{$newskindir}/info.php")) {
        catcherror($lna[174]);
    } else {
        include_once("template/{$newskindir}/info.php");
    }
    ...
    if (!writetofile("data/cache_skinlist.php", $oldcontent . $addline)) {
        ...
        if ($ajax == 'on') {
            ...
            catchsuccess($finishok, $backtoskin);
        }
    }
}

if ($job == 'setskin') {
    if (!$itemid || !file_exists("template/" . basename($itemid) . "/info.php")) {
        catcherror($lna[176]);
    }
...
if ($job == 'removeskin') {
    ...
    if (!writetofile("data/cache_skinlist.php", $content)) {
        catcherror($lna[66] . "data/cache_skinlist.php");
    } else {
        if ($ajax == 'on') {
            $fetchURL = 'admin.php?go=addon_skin';
...
if ($job == 'plugin') {
    $formbody = "  <tr class='admintitle'>
    <td width=30 align=center>{$lna[904]}</td>
    <td align=center width='80%'>{$lna[905]}</td>
    <td align=center>{$lna[0]}</td>
    ...
    for ($i = 0; $i < count($mod_array); $i++) {
        ...
        $formbody .= "  <tr class='$class_distinct'>
  <td width=30 align=center>
    <input type=checkbox name='selid[]' value='{$mod_array[$i]['plid']}' {$chex}></td>
    <td width='80%'>
      <b>{$mod_array[$i]['plname']}</b>$addwords
      <br>
      {$mod_array[$i]['plintro']}
    </td>
    <td align=center width=30>
      <a href='$linkadmin' title='{$lna[0]}'>
        <img src='admin/theme/{$themename}/edit.gif' alt='{$lna[0]}' border='0'>
      </a>
    </td>
    <td align=center width=30>
      <a href='$linkdel' title='{$lna[78]}'>
    ...
    if (count($mod_array) == 0) {
        $formbody .= "  <tr class='sect' align=center>
    <td colspan=4>
      <br>
      <br>
      {$lna[909]}
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
    ...
    <tr class='sect'>
        <td colspan=2 align=center>
            <div align=left>
                <b><font color=red>{$lna[923]}</font></b>
            </div>
            <br>
            {$lna[914]} plugin/<input type=text name=newplugindir size=20>/
...
if ($job == 'addplugin') {
    ...
    if (!$newplugindir || !is_file("plugin/{$newplugindir}/setup.php")) {
        catcherror($lna[915]);
    } else {
        include_once("plugin/{$newplugindir}/setup.php");
    }
    ...
    if ($info['blogversion'] > $blogversion) {
        catcherror($lna[917]);
    }
...
<table width=95% align=center>
  ...
  <tr>
    ...
    <td>
      {$info['author']}
      &nbsp;
      [<a href=\"{$info['authorurl']}\" target=_blank>{$lna[920]}</a>]
    </td>
  </tr>
  <tr>
    <td width=20%>{$lna[921]}</td>
    <td>{$info['version']}</td>
  </tr>
  <tr>
...
<br>
<input type='checkbox' name='activate' value='1' checked>{$lna[922]}\n";
    $display_overall .= highlightadminitems('plugin', 'addon');
    $display_overall .= <<<eot
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
    ...
    <tr class='admintitle'>
        <td align='center'>
            <input type=submit value='{$lna[64]}' class='formbutton'>
             <input type=button onclick='window.location="admin.php?go=addon_plugin";' value='{$lna[138]}' class='formbutton'>
        </td>
    </tr>
</table>
</form>
...
if ($job == 'autoaddplugin') {
    ...
    if (is_file("plugin/{$newplugindir}/admin.php")) {
        $pladmin = 1;
    } else {
        $pladmin = 0;
    }
    ...
    if ($activate == 1 && $info['register']) {
        recache_plugins();
    }

    if (is_file("plugin/{$newplugindir}/install.php")) {
        include_once("plugin/{$newplugindir}/install.php");
    }
...
if ($job == 'pluginsave') {
    ...
    if ($count_selid == 0) {
        $blog->query("UPDATE `{$db_prefix}plugins` SET `active`=0");
    } else {
...
if ($job == 'plugindel') {
    ...
    if (is_file("plugin/{$try['plname']}/uninstall.php")) {
        include_once("plugin/{$try['plname']}/uninstall.php");
    }
    ...
    if (is_file("data/langspecoverwrite.php")) {
        ...
        if (is_array($lncoverwrite)) {
            foreach ($lncoverwrite as $key => $val) {
                ...
                $langexist .= "<div id='editline{$lni}'>
<table width=100% cellpadding=0 cellspacing=0>
  <tr class=\"visibleitem\">
    <td width=15% align=center>
      <input type=text size=4 maxlength=4 name='newlnum[]' id='newlnum{$lni}' onblur='changeldarea({$lni});' value='{$key}'>
    </td>
    <td width=40%>
      <span id='ldarea{$lni}'>{$lnc[$key]}</span>
    </td>
    <td width=40%>
      <input type=text size=52 name='newldesc[]' id='newldesc{$lni}' value=\"{$val}\">
    </td>
    <td width=5% align=center>
      <input type=button onclick='deleteeditline({$lni});' value='{$lna[1149]}' class='formbutton'>
    </td>
  </tr>
</table>
</div>\n";
...
<table width=100% cellpadding=0 cellspacing=0>
    <tr class="admintitle">
        <td width=15% align=center>{$lna[1103]}</td>
        <td width=40%>{$lna[1104]}</td>
        <td width=40%>{$lna[1105]}</td>
...
<div id='editline{$lni}'>
    <table width=100% cellpadding=0 cellspacing=0>
        <tr class="visibleitem">
            <td width=15% align=center>
                <input type=text size=4 maxlength=4 name='newlnum[]' id='newlnum{$lni}' onblur='changeldarea({$lni});'>
            </td>
            <td width=40%>
                <span id='ldarea{$lni}'>&nbsp;</span>
            </td>
            <td width=40%>
                <input type=text size=52 name='newldesc[]' id='newldesc{$lni}'>
            </td>
            <td width=5% align=center>
                <input type=button onclick='deleteeditline({$lni});' value='{$lna[1149]}' class='formbutton'>
            </td>
...
<div align=center>
    <br>
    <input type=button value="{$lna[64]}" class='formbutton' onclick="saveldata();">
    <input type=button onclick='addneweditline();' value="{$lna[1150]}" class='formbutton'> 
    <input type=button onclick='sresetldata();' value="{$lna[65]}" class='formbutton'>
</div>
</form>
...
<table class='tablewidth' align=center cellpadding=4 cellspacing=0>
    <tr>
        <td>
            <b>{$lna[1106]}</b>
            <div style="width:100%; border: 1px solid #ccc; height: 210px; overflow: auto;">
                {$langstext}
            </div>
            <br>
            <br>
