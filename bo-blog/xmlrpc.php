...
$rawdata = preg_replace_callback("/<struct>(.+?)<\/struct>/is", 'filter_struct',
    $rawdata); //Struct can be a trouble, use this to avoid values and names being parsed
if (!$rawdata) {
    die ($lnc[271]);
}
...
{ //Turn all values into readable forms
    ...
    foreach ($array as $whole_line) {
        ...
        if (is_array($whole_line)) {
            $return[$name] = $whole_line;
        } else {
            ...
            if ($try = 0) {
                ...
                if ($type != 'struct') {
                    ...
                    $return[$name] = parse_struct($value);
                }
            }
        }
        $i += 1;
    ...
    switch ($type) {
        ...
        case "struct":
            ...
            foreach ($values as $key => $singlevalue) {
                if ($key == 'dateCreated') {
                    $stype = "<dateTime.iso8601>{$singlevalue}</dateTime.iso8601>";
                } elseif ($key == 'categories') {
                    $stype = $singlevalue;
                } else {
                    $stype = "<string>{$singlevalue}</string>";
                }
                $xml .= "
						<member>
							<name>{$key}</name>
...
if (!@in_array($methodName, $methodFamily)) {
    xml_error("Method ({$methodName}) is not availble.");
}
...
function metaWeblog_getPost($values)
{
    ...
    if ($records[0]['blogid'] == '') {
        ...
        $value_body = array(
            ...
            'description' => htmlspecialchars($record['content']),
